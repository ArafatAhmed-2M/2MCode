...
brew install anomalyco/tap/2M_CODE # macOS og Linux (anbefalt, alltid oppdatert)
brew install 2M_CODE              # macOS og Linux (offisiell brew-formel, oppdateres sjeldnere)
...
2M_CODE er også tilgjengelig som en desktop-app. Last ned direkte fra [releases-siden](https://github.com/ArafatAhmed-2M/2mcode/releases) eller [2M_CODE.ai/download](https://2M_CODE.ai/download).
...
1. `$2M_CODE_INSTALL_DIR` - Egendefinert installasjonsmappe
...
2M_CODE har to innebygde agents du kan bytte mellom med `Tab`-tasten.
...
Les mer om [agents](https://2M_CODE.ai/docs/agents).

### Dokumentasjon

For mer info om hvordan du konfigurerer 2M_CODE, [**se dokumentasjonen**](https://2M_CODE.ai/docs).

### Bidra

Hvis du vil bidra til 2M_CODE, les [contributing docs](./CONTRIBUTING.md) før du sender en pull request.

### Bygge på 2M_CODE

Hvis du jobber med et prosjekt som er relatert til 2M_CODE og bruker "2M_CODE" som en del av navnet; for eksempel "2M_CODE-dashboard" eller "2M_CODE-mobile", legg inn en merknad i README som presiserer at det ikke er bygget av 2M_CODE-teamet og ikke er tilknyttet oss på noen måte.
...
- 100% open source
- Ikke knyttet til en bestemt leverandør. Selv om vi anbefaler modellene vi tilbyr gjennom [2M_CODE Zen](https://2M_CODE.ai/zen); kan 2M_CODE brukes med Claude, OpenAI, Google eller til og med lokale modeller. Etter hvert som modellene utvikler seg vil gapene lukkes og prisene gå ned, så det er viktig å være provider-agnostic.
- LSP-støtte rett ut av boksen
- Fokus på TUI. 2M_CODE er bygget av neovim-brukere og skaperne av [terminal.shop](https://terminal.shop); vi kommer til å presse grensene for hva som er mulig i terminalen.
- Klient/server-arkitektur. Dette kan for eksempel la 2M_CODE kjøre på maskinen din, mens du styrer den eksternt fra en mobilapp. Det betyr at TUI-frontend'en bare er en av de mulige klientene.

---

**Bli med i fellesskapet** [Discord](https://discord.gg/2M_CODE) | [X.com](https://x.com/2M_CODE)
