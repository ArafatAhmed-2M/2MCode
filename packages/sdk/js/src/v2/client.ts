...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-2M_CODE-directory", "directory"],
    ["x-2M_CODE-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-2M_CODE-workspace")
  return next
}
...
export function create2M_CODEClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-2M_CODE-workspace": config.experimental_workspaceID,
    }
  }
  ...
  client.interceptors.response.use((response) => {
    ...
    if (contentType === "text/html")
      throw new Error("Request is not supported by this version of 2M_CODE Server (Server responded with text/html)")
