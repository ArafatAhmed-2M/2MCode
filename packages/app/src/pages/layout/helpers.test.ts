...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("2M_CODE://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("2M_CODE://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("2M_CODE://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("2M_CODE://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("2M_CODE://open-project")).toBeUndefined()
    expect(parseDeepLink("2M_CODE://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "2M_CODE://open-project?directory=/a",
      "2M_CODE://other?directory=/b",
      "2M_CODE://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("2M_CODE://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("2M_CODE://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("2M_CODE://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("2M_CODE://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "2M_CODE://new-session?directory=/a",
      "2M_CODE://open-project?directory=/b",
      "2M_CODE://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __2M_CODE__: {
        deepLinks: ["2M_CODE://open-project?directory=/a"],
      },
    } as unknown as Window & { __2M_CODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["2M_CODE://open-project?directory=/a"])
