...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. 2M_CODE dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u 2M_CODE.
...
:::note
Ovaj alat je dostupan samo kada je `2M_CODE_EXPERIMENTAL_LSP_TOOL=true` (ili `2M_CODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz 2M_CODE provajdera ili kada je varijabla `2M_CODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju 2M_CODE:
