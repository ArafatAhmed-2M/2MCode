...
Postoje dvije vrste agenata u 2M_CODE: primarni agenti i podagenti.
...
2M_CODE dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
2M_CODE dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

2M_CODE dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u 2M_CODE-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `2M_CODE.json`:
...
- Globalno: `~/.config/2M_CODE/agents/`
- Po projektu: `.2M_CODE/agents/`
...
Ako temperatura nije navedena, 2M_CODE koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu 2M_CODE konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj 2M_CODE konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [2M_CODE Zen](/docs/zen), koristili biste `2M_CODE/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `2M_CODE models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/ArafatAhmed-2M/2mcode).
:::
