---
title: Konfiguracija
description: Korištenje 2M_CODE JSON konfiguracije.
---

Možete konfigurirati 2M_CODE koristeći JSON konfiguracijski fajl.

---

## Format

2M_CODE podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/2M_CODE`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/2M_CODE/2M_CODE.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`2M_CODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`2M_CODE.json` u projektu) - postavke specifične za projekat
5. **`.2M_CODE` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`2M_CODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.2M_CODE` i `~/.config/2M_CODE` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/2M_CODE` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu 2M_CODE konfiguraciju u `~/.config/2M_CODE/2M_CODE.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/2M_CODE/tui.json`.
...
Dodajte `2M_CODE.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se 2M_CODE pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `2M_CODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `2M_CODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.2M_CODE` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.2M_CODE`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`2M_CODE.ai/config.json`**](https://2M_CODE.ai/config.json).

TUI konfiguracija koristi [**`2M_CODE.ai/tui.json`**](https://2M_CODE.ai/tui.json).
...
Koristite `2M_CODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `2M_CODE.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `2M_CODE serve` i `2M_CODE web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš 2M_CODE server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `2M_CODE.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj 2M_CODE konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, 2M_CODE pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/2M_CODE/agents/` ili `.2M_CODE/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, 2M_CODE će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`2M_CODE run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/2M_CODE/commands/` ili `.2M_CODE/commands/`. [Saznajte više ovdje](/docs/commands).
...
2M_CODE će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, 2M_CODE **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju 2M_CODE sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.2M_CODE/plugins/` ili `~/.config/2M_CODE/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite 2M_CODE da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
