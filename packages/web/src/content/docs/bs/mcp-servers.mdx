...
Možete dodati vanjske alate u 2M_CODE koristeći _Model Context Protocol_, ili MCP. 2M_CODE podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [2M_CODE Config](https://2M_CODE.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/2M_CODE`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
2M_CODE automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, 2M_CODE će:
...
Ako server zahtijeva autentifikaciju, 2M_CODE će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `2M_CODE mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, 2M_CODE sigurno čuva tokene u `~/.local/share/2M_CODE/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u 2M_CODE, zajedno s ugrađenim alatima. Možete njima upravljati kroz 2M_CODE konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete 2M_CODE sa vašim Sentry nalogom.
