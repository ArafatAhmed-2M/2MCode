---
title: Server
description: Komunicirajte s 2M_CODE serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `2M_CODE serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji 2M_CODE klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `2M_CODE.local` |
...
Postavite `2M_CODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `2M_CODE`, ili postavite `2M_CODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `2M_CODE serve` i za `2M_CODE web`.
...
Kada pokrenete `2M_CODE`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite 2M_CODE server za programsku interakciju sa 2M_CODE-om.
:::

Ova arhitektura omogucava 2M_CODE podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `2M_CODE serve` da startate standalone server. Ako je 2M_CODE TUI vec pokrenut, `2M_CODE serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste 2M_CODE [IDE](/docs/ide) pluginovi.
...
2M_CODE server izlaže sljedece API-je.
