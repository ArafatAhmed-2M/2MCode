...
2M_CODE Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

:::note
2M_CODE Go je trenutno u beta fazi.
:::

Go radi kao bilo koji drugi provajder u 2M_CODE-u. Pretplatite se na 2M_CODE Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili 2M_CODE.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa 2M_CODE-om.
:::
...
2M_CODE Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

2M_CODE Go radi kao bilo koji drugi provajder u 2M_CODE-u.

1. Prijavite se na **<a href={console}>2M_CODE Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `2M_CODE Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na 2M_CODE Go.
:::
...
2M_CODE Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj 2M_CODE konfiguraciji
koristi format `2M_CODE-go/<model-id>`. Na primjer, za Kimi K2.6, koristili biste
`2M_CODE-go/kimi-k2.6` u svojoj konfiguraciji.
...
Napravili smo 2M_CODE Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz 2M_CODE koristite i bilo kojeg drugog provajdera.
