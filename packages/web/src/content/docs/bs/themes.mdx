...
U 2M_CODE mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, 2M_CODE koristi nasu `2M_CODE` temu.
...
2M_CODE dolazi sa vise ugradenih tema.
...
- Zele da 2M_CODE odgovara izgledu njihovog terminala
...
2M_CODE podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/2M_CODE/themes/*.json` ili `$XDG_CONFIG_HOME/2M_CODE/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.2M_CODE/themes/*.json`
4. **Trenutni radni direktorij** - `./.2M_CODE/themes/*.json`
