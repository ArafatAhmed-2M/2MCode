---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u 2M_CODE.
---

import config from "../../../../config.mjs"
export const console = config.console

2M_CODE koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj 2M_CODE konfiguraciji.
...
u `~/.local/share/2M_CODE/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem 2M_CODE
config.
...
2M_CODE Zen je lista modela koje je obezbedio 2M_CODE tim koji su bili
testirano i potvrđeno da dobro radi sa 2M_CODE. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa 2M_CODE Zen.
:::
...
Radi kao i svaki drugi provajder u 2M_CODE i potpuno je opcionalan za korištenje.

---

## 2M_CODE Go

2M_CODE Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža 2M_CODE tim i koji su testirani i verificirani da dobro rade s 2M_CODE-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `2M_CODE Go` i idite na [2M_CODE.ai/auth](https://2M_CODE.ai/zen).
...
Da biste koristili Amazon Bedrock s 2M_CODE:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete 2M_CODE:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `2M_CODE.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u 2M_CODE.
:::
...
Možete konfigurirati 2M_CODE za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi 2M_CODE ispravno radio.
   :::
...
Također možete dodati modele kroz svoju 2M_CODE konfiguraciju.
...
da pohrani token u 2M_CODE auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
2M_CODE koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate 2M_CODE
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`2M_CODE.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `2M_CODE.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s 2M_CODE:
...
Za korištenje Google Vertex AI s 2M_CODE:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće 2M_CODE.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem 2M_CODEa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [2M_CODE-helicone-session](https://github.com/H2Shami/2M_CODE-helicone-session) da automatski prijavite svaki 2M_CODE razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki 2M_CODE razgovor naveden kao zasebna sesija.
...
Možete konfigurirati 2M_CODE za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati 2M_CODE za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati 2M_CODE za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za 2M_CODE. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/2M_CODE) za detalje.
:::
...
Da biste koristili Ollama Cloud s 2M_CODE:
...
3. Kopirajte API ključ za korištenje u 2M_CODE.
...
6. **Važno**: Prije upotrebe modela oblaka u 2M_CODE, morate lokalno povući informacije o modelu:
...
2M_CODE Zen je lista testiranih i verifikovanih modela koju je obezbedio 2M_CODE tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>2M_CODE Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **2M_CODE Zen**.

```txt
   /connect
```

3. Unesite svoj 2M_CODE API ključ.
...
Također možete dodati dodatne modele putem vaše 2M_CODE konfiguracije.
...
5. Također ih možete prilagoditi putem vaše 2M_CODE konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa 2M_CODEom:
...
Također možete prilagoditi modele kroz svoju 2M_CODE konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s 2M_CODE-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `2M_CODE.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju 2M_CODE da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `2M_CODE auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite 2M_CODE konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj 2M_CODE konfiguraciji.
