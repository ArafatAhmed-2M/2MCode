---
title: Regeln
description: Lege eigene Anweisungen fuer 2M_CODE fest.
---

Du kannst 2M_CODE ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in 2M_CODE den Befehl `/init` aus.
...
So kann 2M_CODE sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht 2M_CODE sie zu erweitern.
...
2M_CODE kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/2M_CODE/AGENTS.md` speichern.
Diese gelten in allen 2M_CODE-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt 2M_CODE dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/2M_CODE/AGENTS.md` existiert)
...
Beim Start sucht 2M_CODE in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/2M_CODE/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `2M_CODE.json` oder in `~/.config/2M_CODE/2M_CODE.json` angeben.
...
2M_CODE parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit 2M_CODE.json

Empfohlen ist das `instructions`-Feld in der `2M_CODE.json`:
...
Du kannst 2M_CODE explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `2M_CODE.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
