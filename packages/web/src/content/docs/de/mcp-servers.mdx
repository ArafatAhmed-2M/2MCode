...
Sie können externe Tools zu 2M_CODE hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. 2M_CODE unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [2M_CODE Config](https://2M_CODE.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/2M_CODE`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
2M_CODE übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird 2M_CODE Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von 2M_CODE beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `2M_CODE mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert 2M_CODE die Token sicher in `~/.local/share/2M_CODE/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in 2M_CODE verfügbar. Sie können sie also wie jedes andere Tool über die 2M_CODE-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und 2M_CODE mit Ihrem Sentry-Konto zu verbinden.
