...
2M_CODE Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

:::note
2M_CODE Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Provider in 2M_CODE. Du abonnierst 2M_CODE Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
2M_CODE zu verwenden.
...
2M_CODE Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

2M_CODE Go funktioniert wie jeder andere Provider in 2M_CODE.

1. Du meldest dich bei **<a href={console}>2M_CODE Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `2M_CODE Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann 2M_CODE Go abonnieren.
:::
...
2M_CODE Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner 2M_CODE Config verwendet das Format `2M_CODE-go/<model-id>`. Für Kimi K2.6 würdest du beispielsweise `2M_CODE-go/kimi-k2.6` in deiner Config verwenden.
...
Wir haben 2M_CODE Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit 2M_CODE zu nutzen.
