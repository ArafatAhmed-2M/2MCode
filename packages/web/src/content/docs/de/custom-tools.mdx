...
description: Erstellen Sie Tools, die der LLM in 2M_CODE aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von 2M_CODE wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.2M_CODE/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/2M_CODE/tools/` platzieren.
