...
Es gibt zwei Arten von Agenten in 2M_CODE; Primäragenten und Subagenten.
...
2M_CODE verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `2M_CODE.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet 2M_CODE modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale 2M_CODE-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer 2M_CODE-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [2M_CODE Zen](/docs/zen) verwenden, würden Sie `2M_CODE/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `2M_CODE models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/ArafatAhmed-2M/2mcode).
:::
