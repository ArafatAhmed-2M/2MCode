...
Mit 2M_CODE kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet 2M_CODE das `2M_CODE`-Theme.
...
2M_CODE liefert mehrere eingebaute Themes mit.
...
- 2M_CODE optisch ans Terminal anpassen wollen
...
2M_CODE unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/2M_CODE/themes/*.json` oder `$XDG_CONFIG_HOME/2M_CODE/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.2M_CODE/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.2M_CODE/themes/*.json`
