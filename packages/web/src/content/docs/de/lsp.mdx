---
title: LSP Server
description: 2M_CODE lässt sich in Ihren LSP-Server integrieren.
---

2M_CODE lässt sich in Ihr Language Server Protocol (LSP) integrieren, um die Interaktion von LLM mit Ihrer Codebasis zu unterstützen. Es verwendet Diagnosefunktionen, um dem LLM Feedback zu geben.

---

## Integriert

2M_CODE verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `2M_CODE_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn 2M_CODE eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer 2M_CODE-Konfiguration anpassen.
