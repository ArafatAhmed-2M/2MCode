...
Agent Skills erlauben 2M_CODE, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
2M_CODE sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft 2M_CODE vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.2M_CODE/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/2M_CODE/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.2M_CODE/skills/git-release/SKILL.md` zum Beispiel so:
...
2M_CODE listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `2M_CODE.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `2M_CODE.json`):
