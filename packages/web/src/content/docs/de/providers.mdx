---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in 2M_CODE.
---

import config from "../../../../config.mjs"
export const console = config.console

2M_CODE verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer 2M_CODE-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem 2M_CODE anpassen
config.
...
2M_CODE Zen ist eine Liste von Modellen, die vom 2M_CODE-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit 2M_CODE funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit 2M_CODE Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie 2M_CODE aus und gehen Sie zu [2M_CODE.ai/auth](https://2M_CODE.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in 2M_CODE und ist völlig optional.

---

## 2M_CODE Go

2M_CODE Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom 2M_CODE-Team getestet und verifiziert wurden, dass sie gut mit 2M_CODE funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `2M_CODE Go` und gehen Sie zu [2M_CODE.ai/auth](https://2M_CODE.ai/zen).
...
So verwenden Sie Amazon Bedrock mit 2M_CODE:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie 2M_CODE ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `2M_CODE.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in 2M_CODE wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können 2M_CODE so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit 2M_CODE ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre 2M_CODE-Konfiguration hinzufügen.
...
um das Token im 2M_CODE-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
2M_CODE verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von 2M_CODE
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`2M_CODE.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit 2M_CODE:
...
So verwenden Sie Google Vertex AI mit 2M_CODE:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie 2M_CODE ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über 2M_CODE konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [2M_CODE-helicone-session](https://github.com/H2Shami/2M_CODE-helicone-session)-Plugin, um jede 2M_CODE-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede 2M_CODE-Konversation als separate Sitzung aufgeführt.
...
Sie können 2M_CODE für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können 2M_CODE über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können 2M_CODE über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für 2M_CODE konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/2M_CODE).
:::
...
So verwenden Sie Ollama Cloud mit 2M_CODE:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in 2M_CODE.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in 2M_CODE verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
2M_CODE Zen ist eine Liste getesteter und verifizierter Modelle, die vom 2M_CODE-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>2M_CODE Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **2M_CODE Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren 2M_CODE API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre 2M_CODE-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre 2M_CODE-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit 2M_CODE:
...
Sie können Modelle auch über Ihre 2M_CODE-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit 2M_CODE verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `2M_CODE.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann 2M_CODE erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `2M_CODE auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die 2M_CODE-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer 2M_CODE-Konfiguration übereinstimmt.
