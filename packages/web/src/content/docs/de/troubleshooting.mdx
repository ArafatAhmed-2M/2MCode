...
Wenn 2M_CODE Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\2M_CODE\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `2M_CODE --log-level DEBUG`.

---

## Speicher

2M_CODE speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/2M_CODE/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\2M_CODE`
...
2M_CODE Desktop startet im Hintergrund einen lokalen 2M_CODE-Server (`2M_CODE-cli`-Sidecar).
...
- Nur macOS: `2M_CODE`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/2M_CODE/2M_CODE.jsonc` (oder `~/.config/2M_CODE/2M_CODE.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/2M_CODE/2M_CODE.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\2M_CODE\2M_CODE.jsonc`
...
2M_CODE kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/2M_CODE/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\2M_CODE\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit 2M_CODE ihn neu aufbauen kann.

1. Beende 2M_CODE Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/2M_CODE`
- **Linux**: loesche `~/.cache/2M_CODE` (oder fuehre aus: `rm -rf ~/.cache/2M_CODE`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\2M_CODE`

3. Starte 2M_CODE Desktop neu.

---

### Server-Verbindungsprobleme beheben

2M_CODE Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `2M_CODE.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `2M_CODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `2M_CODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt 2M_CODE Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit 2M_CODE-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

2M_CODE Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer 2M_CODE in den OS-Einstellungen aktiviert sind, und
...
1. Beende 2M_CODE Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von 2M_CODE Desktop):

- `2M_CODE.settings.dat` (desktop default server URL)
- `2M_CODE.global.dat` und `2M_CODE.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit 2M_CODE hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `2M_CODE models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\2M_CODE`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. 2M_CODE installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\2M_CODE`

2. Starte 2M_CODE neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt 2M_CODE, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
2M_CODE erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
