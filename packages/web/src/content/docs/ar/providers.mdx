---
title: المزوّدون
description: استخدام أي موفّر LLM في 2M_CODE.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم 2M_CODE ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات 2M_CODE.
...
في `~/.local/share/2M_CODE/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات 2M_CODE.
...
2M_CODE Zen هي قائمة نماذج يوفّرها فريق 2M_CODE وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع 2M_CODE. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع 2M_CODE Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر 2M_CODE، ثم انتقل إلى [2M_CODE.ai/auth](https://2M_CODE.ai/auth).
...
يعمل مثل أي مزوّد آخر في 2M_CODE واستخدامه اختياري بالكامل.

---

## 2M_CODE Go

2M_CODE Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق 2M_CODE، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع 2M_CODE.

1. شغّل الأمر `/connect` في TUI، واختر `2M_CODE Go`، ثم انتقل إلى [2M_CODE.ai/auth](https://2M_CODE.ai/zen).
...
لاستخدام Amazon Bedrock مع 2M_CODE:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل 2M_CODE:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `2M_CODE.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل 2M_CODE غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين 2M_CODE لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل 2M_CODE بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات 2M_CODE.
...
تخزين الرمز في مساحة تخزين مصادقة 2M_CODE.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم 2M_CODE نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر 2M_CODE
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`2M_CODE.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع 2M_CODE:
...
لاستخدام Google Vertex AI مع 2M_CODE:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر 2M_CODE، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [2M_CODE-helicone-session](https://github.com/H2Shami/2M_CODE-helicone-session) لتسجيل كل محادثة 2M_CODE تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة 2M_CODE مدرجة كجلسة مستقلة.
...
يمكنك ضبط 2M_CODE لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط 2M_CODE لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط 2M_CODE لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع 2M_CODE. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/2M_CODE) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع 2M_CODE:
...
3. انسخ مفتاح API لاستخدامه في 2M_CODE.
...
6. **مهم**: قبل استخدام نماذج السحابة في 2M_CODE، يجب سحب معلومات النموذج محليا:
...
2M_CODE Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق 2M_CODE. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>2M_CODE Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **2M_CODE Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ 2M_CODE.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات 2M_CODE.
...
5. يمكنك أيضا تخصيصها عبر إعدادات 2M_CODE. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع 2M_CODE:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات 2M_CODE. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع 2M_CODE. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `2M_CODE.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ 2M_CODE بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `2M_CODE auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات 2M_CODE وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات 2M_CODE.
