...
مع 2M_CODE يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم 2M_CODE سمتنا `2M_CODE`.
...
يأتي 2M_CODE مع عدة سمات مدمجة.
...
- يريدون أن يطابق 2M_CODE مظهر terminal لديهم
...
يدعم 2M_CODE نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/2M_CODE/themes/*.json` أو `$XDG_CONFIG_HOME/2M_CODE/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.2M_CODE/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.2M_CODE/themes/*.json`
