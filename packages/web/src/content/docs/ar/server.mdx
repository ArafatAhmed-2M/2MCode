---
title: الخادم
description: تفاعل مع خادم 2M_CODE عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `2M_CODE serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل 2M_CODE استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `2M_CODE.local` |
...
عيّن `2M_CODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `2M_CODE`، أو عيّن `2M_CODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `2M_CODE serve` و `2M_CODE web`.
...
عند تشغيل `2M_CODE` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم 2M_CODE للتفاعل مع 2M_CODE برمجيا.
:::

تتيح هذه البنية لـ 2M_CODE دعم عدة عملاء وتمكّنك من التفاعل مع 2M_CODE برمجيا.

يمكنك تشغيل `2M_CODE serve` لبدء خادم مستقل. إذا كانت واجهة 2M_CODE في terminal (TUI)
قيد التشغيل، فسيبدأ `2M_CODE serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات 2M_CODE لـ [IDE](/docs/ide).
...
يعرض خادم 2M_CODE واجهات API التالية.
