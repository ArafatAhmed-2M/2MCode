...
يمكنك إضافة أدوات خارجية إلى 2M_CODE باستخدام _Model Context Protocol_، أو MCP. يدعم 2M_CODE خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات 2M_CODE](https://2M_CODE.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/2M_CODE` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى 2M_CODE تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم 2M_CODE بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك 2M_CODE المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `2M_CODE mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن 2M_CODE الرموز بشكل آمن في `~/.local/share/2M_CODE/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل 2M_CODE إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات 2M_CODE مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط 2M_CODE بحساب Sentry الخاص بك.
