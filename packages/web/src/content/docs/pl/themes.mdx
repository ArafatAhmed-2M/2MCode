...
Dzięki 2M_CODE możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie 2M_CODE używa naszego własnego motywu `2M_CODE`.
...
2M_CODE ma kilka wbudowanych motywów.
...
- Chcą, aby 2M_CODE pasował do wyglądu ich terminala
...
2M_CODE obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/2M_CODE/themes/*.json` lub `$XDG_CONFIG_HOME/2M_CODE/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.2M_CODE/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.2M_CODE/themes/*.json`
