---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć 2M_CODE.
---

Wtyczki umożliwiające wyłączenie 2M_CODE poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania 2M_CODE.
...
- `.2M_CODE/plugins/` - Wtyczki na poziomie projektu
- `~/.config/2M_CODE/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/2M_CODE/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/2M_CODE/2M_CODE.json`)
2. Konfiguracja projektu (`2M_CODE.json`)
3. Globalny katalog wtyczek (`~/.config/2M_CODE/plugins/`)
4. Katalog wtyczek projektu (`.2M_CODE/plugins/`)
...
2M_CODE uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z 2M_CODE do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do 2M_CODE.
...
:::note
Jeśli korzystasz z aplikacji komputerowej 2M_CODE, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj 2M_CODE czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do 2M_CODE:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod 2M_CODE. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://2M_CODE.ai/docs/sdk).
