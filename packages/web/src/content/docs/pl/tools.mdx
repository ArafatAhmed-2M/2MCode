...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. 2M_CODE posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w 2M_CODE.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `2M_CODE_EXPERIMENTAL_LSP_TOOL=true` (lub `2M_CODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy 2M_CODE lub gdy zmienna środowiskowa `2M_CODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu 2M_CODE:
