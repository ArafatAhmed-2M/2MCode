...
Narzędzia specjalistyczne to funkcje, które tworzysz i które LLM może wywoływać podczas rozmów. Współpracują one z [wbudowanymi narzędziami](/docs/tools) 2M_CODE, takimi jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.2M_CODE/tools/` swojego projektu.
- Lub globalnie, umieszczając je w `~/.config/2M_CODE/tools/`.
