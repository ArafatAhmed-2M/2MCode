...
Możesz dodać dodatkowe informacje do 2M_CODE za pomocą _Model Context Protocol_, w skrócie MCP. 2M_CODE obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [2M_CODE Config](https://2M_CODE.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/2M_CODE`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
2M_CODE automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, 2M_CODE:
...
Jeśli serwer wymaga uwierzytelnienia, 2M_CODE poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `2M_CODE mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji 2M_CODE bezpieczne przechowa tokeny w `~/.local/share/2M_CODE/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w 2M_CODE, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie 2M_CODE, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć 2M_CODE z kontem Sentry.
