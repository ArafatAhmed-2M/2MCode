---
title: Serwer
description: Interakcja z serwerem 2M_CODE za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `2M_CODE serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient 2M_CODE.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `2M_CODE.local` |
...
Ustaw `2M_CODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `2M_CODE` lub ustaw `2M_CODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `2M_CODE serve`, jak i `2M_CODE web`.
...
Kiedy uruchomisz `2M_CODE`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera 2M_CODE do programowej interakcji z 2M_CODE.
:::

Ta architektura umożliwia obsługę wielu klientów przez 2M_CODE i programową interakcję z kodem otwartym.

Możesz uruchomić `2M_CODE serve`, aby uruchomić samodzielny serwer. Jeśli masz
2M_CODE TUI działa, `2M_CODE serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki 2M_CODE [IDE](/docs/ide).
...
Serwer 2M_CODE udostępnia następujące interfejsy API.
