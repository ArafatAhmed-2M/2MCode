...
Umiejętności agenta pozwalają 2M_CODE odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
2M_CODE przeszukuje te lokalizacje:

- Project config: `.2M_CODE/skills/<name>/SKILL.md`
- Global config: `~/.config/2M_CODE/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu 2M_CODE przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.2M_CODE/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/2M_CODE/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.2M_CODE/skills/git-release/SKILL.md` w ten sposób:
...
2M_CODE wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `2M_CODE.json`:
...
**Dla agentów wbudowanych** (w `2M_CODE.json`):
