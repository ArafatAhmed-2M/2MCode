...
W 2M_CODE obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
2M_CODE zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
2M_CODE ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

2M_CODE ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej 2M_CODE, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
Skonfiguruj agenta w pliku `2M_CODE.json`:
...
- Globalnie: `~/.config/2M_CODE/agents/`
- W projekcie: `.2M_CODE/agents/`
...
Jeśli nie zostanie skonfigurowana, 2M_CODE używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji 2M_CODE, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji 2M_CODE w `provider/model-id`. Na przykład, [2M_CODE Zen](/docs/zen), użyj `2M_CODE/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `2M_CODE models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/ArafatAhmed-2M/2mcode).
:::
