...
Aby debugować problemy z 2M_CODE, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\2M_CODE\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `2M_CODE --log-level DEBUG`.

---

## Przechowywanie danych

2M_CODE przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/2M_CODE/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\2M_CODE`
...
2M_CODE Desktop uruchamia w tle lokalny serwer 2M_CODE (sidecar `2M_CODE-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `2M_CODE` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/2M_CODE/2M_CODE.jsonc` (lub `~/.config/2M_CODE/2M_CODE.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/2M_CODE/2M_CODE.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\2M_CODE\2M_CODE.jsonc`
...
2M_CODE może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/2M_CODE/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\2M_CODE\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby 2M_CODE mógł ją odbudować.

1. Całkowicie zamknij 2M_CODE Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/2M_CODE`
- **Linux**: usuń `~/.cache/2M_CODE` (lub uruchom `rm -rf ~/.cache/2M_CODE`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\2M_CODE`

3. Uruchom ponownie 2M_CODE Desktop.

---

### Napraw problemy z połączeniem z serwerem

2M_CODE Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `2M_CODE.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `2M_CODE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `2M_CODE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows 2M_CODE Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami 2M_CODE.

---

### Brak powiadomień

2M_CODE Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla 2M_CODE w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie 2M_CODE Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji 2M_CODE Desktop):

- `2M_CODE.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `2M_CODE.global.dat` i `2M_CODE.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `2M_CODE upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `2M_CODE models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\2M_CODE`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. 2M_CODE dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\2M_CODE`

2. Uruchom ponownie kod 2M_CODE, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to 2M_CODE do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
2M_CODE wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
