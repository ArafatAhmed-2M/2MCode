---
title: Konfiguracja
description: Ustawienia 2M_CODE w pliku JSON.
---

Możesz dostosować 2M_CODE za pomocą pliku konfiguracyjnego JSON.

---

## Format

2M_CODE obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "2M_CODE"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/2M_CODE`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/2M_CODE/2M_CODE.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `2M_CODE_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`2M_CODE.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.2M_CODE`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `2M_CODE_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.2M_CODE` i `~/.config/2M_CODE` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/2M_CODE`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację 2M_CODE w `~/.config/2M_CODE/2M_CODE.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/2M_CODE/tui.json`.
...
Dodaj `2M_CODE.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy 2M_CODE się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `2M_CODE_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `2M_CODE_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.2M_CODE` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.2M_CODE`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`2M_CODE.ai/config.json`**](https://2M_CODE.ai/config.json).
...
Użyj `2M_CODE_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `2M_CODE.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `2M_CODE serve` i `2M_CODE web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer 2M_CODE.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `2M_CODE.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie 2M_CODE wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/2M_CODE/agents/` lub `.2M_CODE/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, 2M_CODE powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`2M_CODE run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/2M_CODE/commands/` lub `.2M_CODE/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
2M_CODE może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy 2M_CODE nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie 2M_CODE **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają 2M_CODE o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.2M_CODE/plugins/` lub `~/.config/2M_CODE/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie 2M_CODE tylko do określonych dostawców.
