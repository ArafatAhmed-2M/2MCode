---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w 2M_CODE.
---

import config from "../../../../config.mjs"
export const console = config.console

2M_CODE używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji 2M_CODE.
...
w `~/.local/share/2M_CODE/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim 2M_CODE
config.
...
2M_CODE Zen to lista modeli dostarczonych przez zespół 2M_CODE, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z 2M_CODE. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od 2M_CODE Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `2M_CODE Zen` i przejdź do [2M_CODE.ai/auth](https://2M_CODE.ai/zen).
...
Działa jak każdy inny dostawca w 2M_CODE i jest całkowicie opcjonalny w użyciu.

---

## 2M_CODE Go

2M_CODE Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół 2M_CODE, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z 2M_CODE.

1. Uruchom polecenie `/connect` w TUI, wybierz `2M_CODE Go` i przejdź do [2M_CODE.ai/auth](https://2M_CODE.ai/zen).
...
Aby używać Amazon Bedrock z 2M_CODE:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania 2M_CODE:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `2M_CODE.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w 2M_CODE nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować 2M_CODE do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod 2M_CODE działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji 2M_CODE.
...
2M_CODE używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować 2M_CODE
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`2M_CODE.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem 2M_CODE:
...
Aby używać Google Vertex AI z 2M_CODE:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania 2M_CODE.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą 2M_CODE, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [2M_CODE-helicone-session](https://github.com/H2Shami/2M_CODE-helicone-session), aby automatycznie rejestrować każdą konwersację 2M_CODE jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja 2M_CODE będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować 2M_CODE tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować 2M_CODE tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować 2M_CODE tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem 2M_CODE. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/2M_CODE).
:::
...
Aby korzystać z Ollama Cloud z 2M_CODE:
...
3. Skopiuj klucz API do użycia w 2M_CODE.
...
6. **Ważne**: Przed użyciem modeli chmurowych w 2M_CODE musisz pobrać informacje o modelu lokalnie:
...
2M_CODE Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół 2M_CODE. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>2M_CODE Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **2M_CODE Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API 2M_CODE.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji 2M_CODE.
...
5. Możesz je także dostosować za pomocą konfiguracji 2M_CODE. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z 2M_CODE:
...
Możesz także dostosować modele za pomocą konfiguracji 2M_CODE. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z 2M_CODE. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `2M_CODE.json` w katalogu projektu:
...
Pola `limit` pozwalają 2M_CODE zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `2M_CODE auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację 2M_CODE i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji 2M_CODE.
