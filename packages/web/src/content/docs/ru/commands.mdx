...
Создайте `.2M_CODE/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию 2M_CODE или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем 2M_CODE [config](/docs/config):
...
- Глобальный: `~/.config/2M_CODE/commands/`
- Для каждого проекта: `.2M_CODE/commands/`
...
2M_CODE включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
