...
2M_CODE Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

:::note
2M_CODE Go is currently in beta.
:::

Go works like any other provider in 2M_CODE. You subscribe to 2M_CODE Go and
...
2M_CODE Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

2M_CODE Go works like any other provider in 2M_CODE.

1. You sign in to **<a href={console}>2M_CODE Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `2M_CODE Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to 2M_CODE Go.
:::
...
2M_CODE Go includes the following limits:
...
uses the format `2M_CODE-go/<model-id>`. For example, for Kimi K2.6, you would
use `2M_CODE-go/kimi-k2.6` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with 2M_CODE as well.
