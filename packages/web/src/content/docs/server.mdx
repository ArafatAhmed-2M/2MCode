---
title: Server
description: Interact with 2M_CODE server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `2M_CODE serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an 2M_CODE client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `2M_CODE.local` |
...
Set `2M_CODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `2M_CODE`, or set `2M_CODE_SERVER_USERNAME` to override it. This applies to both `2M_CODE serve` and `2M_CODE web`.
...
When you run `2M_CODE` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the 2M_CODE server to interact with 2M_CODE programmatically.
:::

This architecture lets 2M_CODE support multiple clients and allows you to interact with 2M_CODE programmatically.

You can run `2M_CODE serve` to start a standalone server. If you have the
2M_CODE TUI running, `2M_CODE serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the 2M_CODE [IDE](/docs/ide) plugins.
...
The 2M_CODE server exposes the following APIs.
