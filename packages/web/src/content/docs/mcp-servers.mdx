...
You can add external tools to 2M_CODE using the _Model Context Protocol_, or MCP. 2M_CODE supports both local and remote servers.
...
You can define MCP servers in your [2M_CODE Config](https://2M_CODE.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/2M_CODE` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
2M_CODE automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, 2M_CODE will:
...
If the server requires authentication, 2M_CODE will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `2M_CODE mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, 2M_CODE will store the tokens securely in `~/.local/share/2M_CODE/mcp-auth.json`.
...
Your MCPs are available as tools in 2M_CODE, alongside built-in tools. So you can manage them through the 2M_CODE config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect 2M_CODE to your Sentry account.
