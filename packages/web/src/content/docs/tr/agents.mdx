...
2M_CODE'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
2M_CODE, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
2M_CODE, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

2M_CODE iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `2M_CODE.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse 2M_CODE modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global 2M_CODE yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
2M_CODE yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [2M_CODE Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `2M_CODE/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `2M_CODE models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/ArafatAhmed-2M/2mcode).
:::
