...
2M_CODE ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak 2M_CODE kendi `2M_CODE` temasını kullanır.
...
2M_CODE birden fazla yerleşik temayla gelir.
...
- 2M_CODE'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
2M_CODE, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/2M_CODE/themes/*.json` veya `$XDG_CONFIG_HOME/2M_CODE/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.2M_CODE/themes/*.json`
4. **Geçerli çalışma dizini** - `./.2M_CODE/themes/*.json`
