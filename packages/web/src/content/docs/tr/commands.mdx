...
`.2M_CODE/commands/test.md` oluştur:
...
2M_CODE işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

2M_CODE [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/2M_CODE/commands/`
- Proje başına: `.2M_CODE/commands/`
...
2M_CODE, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
