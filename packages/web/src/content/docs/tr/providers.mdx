---
title: Sağlayıcılar
description: 2M_CODE'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

2M_CODE, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı 2M_CODE yapılandırmanızda yapılandırın.
...
`~/.local/share/2M_CODE/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları 2M_CODE'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
2M_CODE Zen, 2M_CODE ekibi tarafından sağlanan modellerin bir listesidir.
2M_CODE ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz 2M_CODE Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, 2M_CODE'u seçin ve [2M_CODE.ai/auth](https://2M_CODE.ai/auth)'ye gidin.
...
2M_CODE'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## 2M_CODE Go

2M_CODE Go, 2M_CODE ile iyi çalıştığı test edilmiş ve doğrulanmış, 2M_CODE ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `2M_CODE Go`'yu seçin ve [2M_CODE.ai/auth](https://2M_CODE.ai/zen) adresine gidin.
...
Amazon Bedrock'u 2M_CODE ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   2M_CODE'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `2M_CODE.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi 2M_CODE'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2M_CODE'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   2M_CODE'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   2M_CODE yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
2M_CODE, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. 2M_CODE'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`2M_CODE.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`2M_CODE.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi 2M_CODE'la kullanmak için:
...
Google Vertex AI'yi 2M_CODE ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları 2M_CODE çalıştırılırken ayarlayın.
...
Helicone'un 2M_CODE aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her 2M_CODE görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [2M_CODE-helicone-session](https://github.com/H2Shami/2M_CODE-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her 2M_CODE görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
2M_CODE'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla 2M_CODE'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla 2M_CODE'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini 2M_CODE için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/2M_CODE)'a bakın.
:::
...
Ollama Cloud'u 2M_CODE ile kullanmak için:
...
3. 2M_CODE'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: 2M_CODE'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
2M_CODE Zen, 2M_CODE ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>2M_CODE Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **2M_CODE Zen**'i arayın.

   ```txt
   /connect
   ```

3. 2M_CODE API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   2M_CODE yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca 2M_CODE yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi 2M_CODE ile kullanmak için:
...
Modelleri 2M_CODE yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı 2M_CODE ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `2M_CODE.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları 2M_CODE'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `2M_CODE auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için 2M_CODE yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, 2M_CODE yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
