---
title: TUI
description: 2M_CODE terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

2M_CODE, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

2M_CODE'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
2M_CODE TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
2M_CODE'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
2M_CODE'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `2M_CODE.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `2M_CODE_TUI_CONFIG` kullanın.
