---
title: Eklentiler
description: 2M_CODE'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek 2M_CODE'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya 2M_CODE'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.2M_CODE/plugins/` - Proje düzeyinde eklentiler
- `~/.config/2M_CODE/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/2M_CODE/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/2M_CODE/2M_CODE.json`)
2. Project config (`2M_CODE.json`)
3. Global eklenti dizini (`~/.config/2M_CODE/plugins/`)
4. Proje eklenti dizini (`.2M_CODE/plugins/`)
...
2M_CODE bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir 2M_CODE SDK istemcisi.
...
2M_CODE'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
2M_CODE masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
2M_CODE'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca 2M_CODE'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, 2M_CODE'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra 2M_CODE'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://2M_CODE.ai/docs/sdk) bakın.
