...
_Model Bağlam Protokolü_ veya MCP kullanarak 2M_CODE'a harici araçlar ekleyebilirsiniz. 2M_CODE hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [2M_CODE Config](https://2M_CODE.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/2M_CODE` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
2M_CODE, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde 2M_CODE şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa 2M_CODE, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `2M_CODE mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından 2M_CODE, belirteçleri `~/.local/share/2M_CODE/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra 2M_CODE'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi 2M_CODE yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve 2M_CODE'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
