---
title: Sunucu
description: 2M_CODE sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`2M_CODE serve` komutu, 2M_CODE istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `2M_CODE.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `2M_CODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `2M_CODE` değeridir; değiştirmek isterseniz `2M_CODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `2M_CODE serve` hem de `2M_CODE web` için geçerlidir.
...
`2M_CODE` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
2M_CODE ile programatik etkileşim için 2M_CODE sunucusunu kullanın.
:::

Bu mimari, 2M_CODE'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`2M_CODE serve` ile bağımsız bir sunucu başlatabilirsiniz. 2M_CODE TUI açıksa, `2M_CODE serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum 2M_CODE [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
2M_CODE sunucusu aşağıdaki API'leri sunar.
