...
Les compétences d'agent permettent à 2M_CODE de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
2M_CODE recherche ces emplacements :

- Configuration du projet : `.2M_CODE/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/2M_CODE/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, 2M_CODE remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.2M_CODE/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/2M_CODE/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.2M_CODE/skills/git-release/SKILL.md` comme ceci :
...
2M_CODE répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `2M_CODE.json` :
...
**Pour les agents intégrés** (dans `2M_CODE.json`) :
