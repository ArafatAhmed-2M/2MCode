---
title: SDK
description: Client JS de type sécurisé pour le serveur 2M_CODE.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK 2M_CODE JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler 2M_CODE par programme.
...
Créez une instance de 2M_CODE :

```javascript
import { create2M_CODE } from "@2M_CODE-ai/sdk"

const { client } = await create2M_CODE()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `2M_CODE.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${2M_CODE.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de 2M_CODE, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@2M_CODE-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*2M_CODE" },
})
