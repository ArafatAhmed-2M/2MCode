...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez 2M_CODE.
...
Voici plusieurs modèles qui fonctionnent bien avec 2M_CODE, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration 2M_CODE.

```json title="2M_CODE.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [2M_CODE Zen](/docs/zen), vous utiliserez `2M_CODE/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="2M_CODE.jsonc" {7-12,19-24}
{
  "$schema": "https://2M_CODE.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="2M_CODE.jsonc" {6-21}
{
  "$schema": "https://2M_CODE.ai/config.json",
  "provider": {
    "2M_CODE": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. 2M_CODE est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

2M_CODE est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="2M_CODE.jsonc" {7-18}
{
  "$schema": "https://2M_CODE.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque 2M_CODE démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration 2M_CODE.
