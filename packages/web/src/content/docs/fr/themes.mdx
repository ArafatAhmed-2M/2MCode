...
Avec 2M_CODE, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, 2M_CODE utilise notre propre thème `2M_CODE`.
...
2M_CODE est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que 2M_CODE corresponde à l'apparence de leur terminal
...
2M_CODE prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/2M_CODE/themes/*.json` ou `$XDG_CONFIG_HOME/2M_CODE/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.2M_CODE/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.2M_CODE/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/2M_CODE/themes
vim ~/.config/2M_CODE/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .2M_CODE/themes
vim .2M_CODE/themes/my-theme.json
```
...
{
  "$schema": "https://2M_CODE.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
