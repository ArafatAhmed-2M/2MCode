---
title: Plugins
description: Écrivez vos propres plugins pour étendre 2M_CODE.
---

Les plugins vous permettent d'étendre 2M_CODE en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de 2M_CODE.
...
- `.2M_CODE/plugins/` - Plugins au niveau du projet
- `~/.config/2M_CODE/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/2M_CODE/node_modules/`.
...
1. Configuration globale (`~/.config/2M_CODE/2M_CODE.json`)
2. Configuration du projet (`2M_CODE.json`)
3. Répertoire global des plugins (`~/.config/2M_CODE/plugins/`)
4. Répertoire des plugins du projet (`.2M_CODE/plugins/`)
...
2M_CODE exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK 2M_CODE pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre 2M_CODE.
...
:::note
Si vous utilisez l'application de bureau 2M_CODE, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez 2M_CODE de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à 2M_CODE :
...
L'assistant `tool` crée un outil personnalisé que 2M_CODE peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour 2M_CODE aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://2M_CODE.ai/docs/sdk) pour plus de détails.
