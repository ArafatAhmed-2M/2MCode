---
title: Serveur
description: Interagissez avec le serveur 2M_CODE via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `2M_CODE serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client 2M_CODE peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `2M_CODE.local` |
...
Définissez `2M_CODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `2M_CODE`, ou définissez `2M_CODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `2M_CODE serve` et à `2M_CODE web`.
...
Lorsque vous exécutez `2M_CODE`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur 2M_CODE pour interagir avec 2M_CODE par programmation.
:::

Cette architecture permet à 2M_CODE de prendre en charge plusieurs clients et vous permet d'interagir avec 2M_CODE par programme.

Vous pouvez exécuter `2M_CODE serve` pour démarrer un serveur autonome. Si vous avez le
2M_CODE TUI en cours d'exécution, `2M_CODE serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins 2M_CODE [IDE](/docs/ide).
...
Le serveur 2M_CODE expose les API suivantes.
