---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en 2M_CODE.
---

import config from "../../../../config.mjs"
export const console = config.console

2M_CODE utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration 2M_CODE.
...
en `~/.local/share/2M_CODE/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre 2M_CODE
configuration.
...
2M_CODE Zen est une liste de modèles fournis par l'équipe 2M_CODE qui ont été
testé et vérifié pour fonctionner correctement avec 2M_CODE. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par 2M_CODE Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez 2M_CODE et dirigez-vous vers [2M_CODE.ai/auth](https://2M_CODE.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans 2M_CODE et son utilisation est totalement facultative.

---

## 2M_CODE Go

2M_CODE Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe 2M_CODE qui ont été
testé et vérifié pour fonctionner correctement avec 2M_CODE.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `2M_CODE Go` et rendez-vous sur [2M_CODE.ai/auth](https://2M_CODE.ai/zen).
...
Pour utiliser Amazon Bedrock avec 2M_CODE :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de 2M_CODE :
...
Pour une configuration spécifique au projet ou persistante, utilisez `2M_CODE.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans 2M_CODE n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer 2M_CODE pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que 2M_CODE fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration 2M_CODE.
...
pour stocker le jeton dans le stockage d'authentification 2M_CODE.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
2M_CODE utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller 2M_CODE
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `2M_CODE.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `2M_CODE.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec 2M_CODE :
...
Pour utiliser Google Vertex AI avec 2M_CODE :
...
Définissez-les lors de l'exécution de 2M_CODE.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via 2M_CODE, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [2M_CODE-helicone-session](https://github.com/H2Shami/2M_CODE-helicone-session) pour enregistrer automatiquement chaque conversation 2M_CODE en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation 2M_CODE répertoriée comme une session distincte.
...
Vous pouvez configurer 2M_CODE pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer 2M_CODE pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer 2M_CODE pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour 2M_CODE. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/2M_CODE) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec 2M_CODE :
...
3. Copiez la clé API à utiliser dans 2M_CODE.
...
6. **Important** : Avant d'utiliser des modèles cloud dans 2M_CODE, vous devez extraire les informations du modèle localement :
...
2M_CODE Zen est une liste de modèles testés et vérifiés fournie par l'équipe 2M_CODE. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>2M_CODE Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **2M_CODE Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé 2M_CODE API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration 2M_CODE.
...
5. Vous pouvez également les personnaliser via votre configuration 2M_CODE. Voici un exemple de spécification d'un fournisseur
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec 2M_CODE :
...
Vous pouvez également personnaliser les modèles via votre configuration 2M_CODE. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec 2M_CODE. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `2M_CODE.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à 2M_CODE de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `2M_CODE auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration 2M_CODE et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration 2M_CODE.
