...
Vous pouvez ajouter des outils externes à 2M_CODE à l'aide du _Model Context Protocol_ ou MCP. 2M_CODE prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [2M_CODE Config](https://2M_CODE.ai/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/2M_CODE`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
2M_CODE gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, 2M_CODE :
...
Si le serveur nécessite une authentification, 2M_CODE vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `2M_CODE mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, 2M_CODE stockera les jetons en toute sécurité dans `~/.local/share/2M_CODE/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans 2M_CODE, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration 2M_CODE comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter 2M_CODE à votre compte Sentry.
