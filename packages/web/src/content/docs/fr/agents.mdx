...
Il existe deux types d'agents dans 2M_CODE ; agents primaires et sous-agents.
...
2M_CODE est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
2M_CODE est livré avec trois sous-agents intégrés, **General**, **Explore** et **Scout**. Nous les verrons ci-dessous.

---

## Agents intégrés

2M_CODE est livré avec deux agents primaires intégrés et trois sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `2M_CODE.json` :
...
Si aucune température n'est spécifiée, 2M_CODE utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale 2M_CODE et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration 2M_CODE utilise le format `provider/model-id`. Par exemple, si vous utilisez [2M_CODE Zen](/docs/zen), vous utiliserez `2M_CODE/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `2M_CODE models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/ArafatAhmed-2M/2mcode).
:::
