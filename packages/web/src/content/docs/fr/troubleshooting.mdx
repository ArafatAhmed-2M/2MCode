...
Pour déboguer les problèmes avec 2M_CODE, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\2M_CODE\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `2M_CODE --log-level DEBUG`.

---

## Stockage

2M_CODE stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/2M_CODE/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\2M_CODE`
...
2M_CODE Desktop exécute un serveur 2M_CODE local (le side-car `2M_CODE-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `2M_CODE` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/2M_CODE/2M_CODE.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\2M_CODE\2M_CODE.jsonc`
...
2M_CODE peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/2M_CODE/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\2M_CODE\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que 2M_CODE puisse le reconstruire.

1. Quittez complètement 2M_CODE Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/2M_CODE`
- **Linux** : supprimez `~/.cache/2M_CODE` (ou exécutez `rm -rf ~/.cache/2M_CODE`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\2M_CODE`

3. Redémarrez le bureau 2M_CODE.

---

### Résoudre les problèmes de connexion au serveur

2M_CODE Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `2M_CODE.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `2M_CODE_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `2M_CODE_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, 2M_CODE Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de 2M_CODE.

---

### Les notifications ne s'affichent pas

2M_CODE Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour 2M_CODE dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau 2M_CODE.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application 2M_CODE Desktop) :

- `2M_CODE.settings.dat` (serveur de bureau par défaut URL)
- `2M_CODE.global.dat` et `2M_CODE.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec 2M_CODE :
...
3. Assurez-vous d'avoir la dernière version avec `2M_CODE upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `2M_CODE models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\2M_CODE`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. 2M_CODE installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\2M_CODE`

2. Redémarrez 2M_CODE pour réinstaller les derniers packages du fournisseur

Cela forcera 2M_CODE à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
2M_CODE détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
