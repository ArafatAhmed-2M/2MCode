...
Les outils permettent au LLM d'effectuer des actions dans votre base de code. 2M_CODE est livré avec un ensemble d'outils intégrés, mais vous pouvez l'étendre avec [outils personnalisés](/docs/custom-tools) ou [serveurs MCP](/docs/mcp-servers).
...
Voici tous les outils intégrés disponibles dans 2M_CODE.
...
:::note
Cet outil n'est disponible que lorsque `2M_CODE_EXPERIMENTAL_LSP_TOOL=true` (ou `2M_CODE_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur 2M_CODE ou lorsque la variable d'environnement `2M_CODE_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).

À activer lors du lancement de 2M_CODE :
