---
title: Config
description: Using the 2M_CODE JSON config.
---

You can configure 2M_CODE using a JSON config file.

---

## Format

2M_CODE supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
1. **Remote config** (from `.well-known/2M_CODE`) - organizational defaults
2. **Global config** (`~/.config/2M_CODE/2M_CODE.json`) - user preferences
3. **Custom config** (`2M_CODE_CONFIG` env var) - custom overrides
4. **Project config** (`2M_CODE.json` in project) - project-specific settings
5. **`.2M_CODE` directories** - agents, commands, plugins
6. **Inline config** (`2M_CODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/2M_CODE/` on macOS) - admin-controlled
...
:::note
The `.2M_CODE` and `~/.config/2M_CODE` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/2M_CODE` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
Place your global 2M_CODE config in `~/.config/2M_CODE/2M_CODE.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/2M_CODE/tui.json`.
...
Add `2M_CODE.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When 2M_CODE starts up, it looks for a config file in the current directory or traverse up to the nearest Git directory.
...
Specify a custom config file path using the `2M_CODE_CONFIG` environment variable.
...
Specify a custom config directory using the `2M_CODE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.2M_CODE` directory, and should
...
The custom directory is loaded after the global config and `.2M_CODE` directories, so it **can override** their settings.
...
Drop an `2M_CODE.json` or `2M_CODE.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/2M_CODE/` |
| Linux    | `/etc/2M_CODE/`                         |
| Windows  | `%ProgramData%\2M_CODE`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, 2M_CODE reads managed preferences from the `ai.2M_CODE.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

2M_CODE checks these paths:

1. `/Library/Managed Preferences/<user>/ai.2M_CODE.managed.plist`
2. `/Library/Managed Preferences/ai.2M_CODE.managed.plist`

The plist keys map directly to `2M_CODE.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.2M_CODE.managed` PayloadType. The 2M_CODE config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.2M_CODE.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.2M_CODE.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.2M_CODE</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`2M_CODE.ai/config.json`**](https://2M_CODE.ai/config.json).

TUI config uses [**`2M_CODE.ai/tui.json`**](https://2M_CODE.ai/tui.json).
...
Use `2M_CODE_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `2M_CODE.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `2M_CODE serve` and `2M_CODE web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your 2M_CODE server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `2M_CODE.local`. Useful for running multiple instances on the same network.
...
If not specified, 2M_CODE will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your 2M_CODE config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, 2M_CODE tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
2M_CODE normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, 2M_CODE omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/2M_CODE/agents/` or `.2M_CODE/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, 2M_CODE will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`2M_CODE run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/2M_CODE/commands/` or `.2M_CODE/commands/`. [Learn more here](/docs/commands).
...
2M_CODE uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
2M_CODE will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, 2M_CODE **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend 2M_CODE with custom tools, hooks, and integrations.

Place plugin files in `.2M_CODE/plugins/` or `~/.config/2M_CODE/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict 2M_CODE to only use specific providers rather than disabling them one by one.
