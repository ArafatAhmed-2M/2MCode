---
title: Tjener
description: Samhandle med 2M_CODE-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `2M_CODE serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en 2M_CODE-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `2M_CODE.local` |
...
Sett `2M_CODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `2M_CODE` som standard, eller sett `2M_CODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `2M_CODE serve` og `2M_CODE web`.
...
Når du kjører `2M_CODE` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk 2M_CODE-serveren til å samhandle med 2M_CODE programmatisk.
:::

Denne arkitekturen lar 2M_CODE støtte flere klienter og lar deg samhandle med 2M_CODE programmatisk.

Du kan kjøre `2M_CODE serve` for å starte en frittstående server. Hvis du har
2M_CODE TUI kjører, vil `2M_CODE serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av 2M_CODE [IDE](/docs/ide) plugins.
...
2M_CODE-serveren viser følgende APIer.
