...
Du kan legge til eksterne verktøy i 2M_CODE ved å bruke _Model Context Protocol_ eller MCP. 2M_CODE støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [2M_CODE Config](https://2M_CODE.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="2M_CODE.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/2M_CODE`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
2M_CODE håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil 2M_CODE:
...
Hvis serveren krever autentisering, vil 2M_CODE be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `2M_CODE mcp auth <server-name>`.
...
```bash
2M_CODE mcp auth my-oauth-server
```
...
```bash
2M_CODE mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil 2M_CODE lagre tokens sikkert i `~/.local/share/2M_CODE/mcp-auth.json`.
...
2M_CODE mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i 2M_CODE, sammen med innebygde verktøy. Så du kan administrere dem gjennom 2M_CODE-konfigurasjonen som alle andre verktøy.
...
```json title="2M_CODE.json" {11, 14-18}
{
  "$schema": "https://2M_CODE.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
2M_CODE mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble 2M_CODE til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver 2M_CODE-samtale oppført som en separat økt.
