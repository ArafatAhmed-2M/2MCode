...
description: Skriv dine egne programtillegg for å utvide 2M_CODE.
---

Plugins lar deg utvide 2M_CODE ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til 2M_CODE.
...
- `.2M_CODE/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/2M_CODE/node_modules/`.
...
1. Global konfigurasjon (`~/.config/2M_CODE/2M_CODE.json`)
2. Prosjektkonfigurasjon (`2M_CODE.json`)
3. Global plugin-katalog (`~/.config/2M_CODE/plugins/`)
4. Prosjektpluginkatalog (`.2M_CODE/plugins/`)
...
2M_CODE kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En 2M_CODE SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide 2M_CODE.
...
:::note
Hvis du bruker 2M_CODE-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre 2M_CODE fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til 2M_CODE:
...
`tool`-hjelperen lager et tilpasset verktøy som 2M_CODE kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for 2M_CODE sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://2M_CODE.ai/docs/sdk) for detaljer.
