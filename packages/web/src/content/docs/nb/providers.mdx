---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i 2M_CODE.
---

import config from "../../../../config.mjs"
export const console = config.console

2M_CODE bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i 2M_CODE-konfigurasjonen.
...
i `~/.local/share/2M_CODE/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i 2M_CODE
konfig.
...
2M_CODE Zen er en liste over modeller levert av 2M_CODE-teamet som har vært
testet og verifisert for å fungere godt med 2M_CODE. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med 2M_CODE Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `2M_CODE Zen` og gå til [2M_CODE.ai/auth](https://2M_CODE.ai/zen).
...
Det fungerer som alle andre leverandører i 2M_CODE og er helt valgfritt å bruke.

---

## 2M_CODE Go

2M_CODE Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av 2M_CODE-teamet som har vært
testet og verifisert for å fungere godt med 2M_CODE.

1. Kjør kommandoen `/connect` i TUI, velg `2M_CODE Go`, og gå til [2M_CODE.ai/auth](https://2M_CODE.ai/zen).
...
Slik bruker du Amazon Bedrock med 2M_CODE:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører 2M_CODE:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `2M_CODE.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i 2M_CODE støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere 2M_CODE til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at 2M_CODE skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom 2M_CODE-konfigurasjonen.
...
for å lagre token i 2M_CODE auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
2M_CODE bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse 2M_CODE
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`2M_CODE.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med 2M_CODE:
...
Slik bruker du Google Vertex AI med 2M_CODE:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører 2M_CODE.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom 2M_CODE, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [2M_CODE-helicone-session](https://github.com/H2Shami/2M_CODE-helicone-session) plugin for automatisk å logge hver 2M_CODE-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver 2M_CODE-samtale oppført som en separat økt.
...
Du kan konfigurere 2M_CODE for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere 2M_CODE for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere 2M_CODE for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for 2M_CODE. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/2M_CODE) for detaljer.
:::
...
Slik bruker du Ollama Cloud med 2M_CODE:
...
3. Kopier API-nøkkelen for bruk i 2M_CODE.
...
6. **Viktig**: Før du bruker skymodeller i 2M_CODE, må du hente modellinformasjonen lokalt:
...
2M_CODE Zen er en liste over testede og verifiserte modeller levert av 2M_CODE-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>2M_CODE Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **2M_CODE Zen**.

   ```txt
   /connect
   ```

3. Skriv inn 2M_CODE API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom 2M_CODE-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom 2M_CODE-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med 2M_CODE:
...
Du kan også tilpasse modeller gjennom 2M_CODE-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med 2M_CODE. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `2M_CODE.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar 2M_CODE forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `2M_CODE auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk 2M_CODE-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i 2M_CODE-konfigurasjonen.
