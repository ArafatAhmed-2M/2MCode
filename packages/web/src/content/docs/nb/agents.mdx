...
Det er to typer agenter i 2M_CODE; primære agenter og underagenter.
...
2M_CODE kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
2M_CODE kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

2M_CODE kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `2M_CODE.json`:
...
Hvis ingen temperatur er spesifisert, bruker 2M_CODE modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale 2M_CODE-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i 2M_CODE-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [2M_CODE Zen](/docs/zen), vil du bruke `2M_CODE/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `2M_CODE models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/ArafatAhmed-2M/2mcode).
:::
