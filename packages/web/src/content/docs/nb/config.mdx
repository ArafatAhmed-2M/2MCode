---
title: Konfigurasjon
description: Ved å bruke 2M_CODE JSON-konfigurasjonen.
---

Du kan konfigurere 2M_CODE ved å bruke en JSON konfigurasjonsfil.

---

## Format

2M_CODE støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "2M_CODE"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/2M_CODE`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/2M_CODE/2M_CODE.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`2M_CODE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`2M_CODE.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.2M_CODE` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`2M_CODE_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.2M_CODE` og `~/.config/2M_CODE` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/2M_CODE`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale 2M_CODE-konfigurasjon i `~/.config/2M_CODE/2M_CODE.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `2M_CODE.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når 2M_CODE starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `2M_CODE_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `2M_CODE_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.2M_CODE` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.2M_CODE`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`2M_CODE.ai/config.json`**](https://2M_CODE.ai/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `2M_CODE serve` og `2M_CODE web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage 2M_CODE-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `2M_CODE.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i 2M_CODE-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver 2M_CODE å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/2M_CODE/agents/` eller `.2M_CODE/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil 2M_CODE falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`2M_CODE run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/2M_CODE/commands/` eller `.2M_CODE/commands/`. [Les mer her](/docs/commands).
...
2M_CODE vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater 2M_CODE alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider 2M_CODE med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.2M_CODE/plugins/` eller `~/.config/2M_CODE/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense 2M_CODE til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
