...
Você pode adicionar ferramentas externas ao 2M_CODE usando o _Model Context Protocol_, ou MCP. O 2M_CODE suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do 2M_CODE](https://2M_CODE.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/2M_CODE`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O 2M_CODE lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o 2M_CODE irá:
...
Se o servidor requer autenticação, o 2M_CODE solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `2M_CODE mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o 2M_CODE armazenará os tokens de forma segura em `~/.local/share/2M_CODE/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no 2M_CODE, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do 2M_CODE como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o 2M_CODE à sua conta do Sentry.
