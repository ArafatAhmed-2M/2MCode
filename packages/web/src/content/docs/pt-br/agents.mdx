...
Existem dois tipos de agentes no 2M_CODE; agentes primários e subagentes.
...
2M_CODE vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
2M_CODE vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

2M_CODE vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do 2M_CODE, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `2M_CODE.json`:
...
- Global: `~/.config/2M_CODE/agents/`
- Por projeto: `.2M_CODE/agents/`

```markdown title="~/.config/2M_CODE/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o 2M_CODE usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do 2M_CODE quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do 2M_CODE usa o formato `provider/model-id`. Por exemplo, se você estiver usando [2M_CODE Zen](/docs/zen), você usaria `2M_CODE/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="2M_CODE.json" {3-6,9-12}
{
  "$schema": "https://2M_CODE.ai/config.json",
  "tools": {
    "write": true,
...
```json title="2M_CODE.json" {3-5,8-10}
{
  "$schema": "https://2M_CODE.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `2M_CODE models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/ArafatAhmed-2M/2mcode).
:::
...
```markdown title="~/.config/2M_CODE/agents/docs-writer.md"
---
...
```markdown title="~/.config/2M_CODE/agents/security-auditor.md"
---
