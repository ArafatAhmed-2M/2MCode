---
title: Servidor
description: Interaja com o servidor 2M_CODE via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `2M_CODE serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente 2M_CODE pode usar.
...
```bash
2M_CODE serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `2M_CODE.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
2M_CODE serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `2M_CODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `2M_CODE`, ou defina `2M_CODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `2M_CODE serve` quanto ao `2M_CODE web`.

```bash
2M_CODE_SERVER_PASSWORD=your-password 2M_CODE serve
```
...
Quando você executa `2M_CODE`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor 2M_CODE para interagir com o 2M_CODE programaticamente.
:::

Essa arquitetura permite que o 2M_CODE suporte múltiplos clientes e permite que você interaja com o 2M_CODE programaticamente.

Você pode executar `2M_CODE serve` para iniciar um servidor autônomo. Se você tiver o TUI do 2M_CODE em execução, `2M_CODE serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do 2M_CODE [IDE](/docs/ide).
...
O servidor 2M_CODE expõe as seguintes APIs.
