...
As habilidades do agente permitem que o 2M_CODE descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O 2M_CODE pesquisa nesses locais:

- Configuração do projeto: `.2M_CODE/skills/<name>/SKILL.md`
- Configuração global: `~/.config/2M_CODE/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o 2M_CODE sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.2M_CODE/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/2M_CODE/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.2M_CODE/skills/git-release/SKILL.md` assim:
...
O 2M_CODE lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `2M_CODE.json`:
...
**Para agentes embutidos** (em `2M_CODE.json`):
