...
Com o 2M_CODE, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o 2M_CODE usa nosso próprio tema `2M_CODE`.
...
O 2M_CODE vem com vários temas embutidos.
...
- Querem que o 2M_CODE corresponda à aparência do seu terminal
...
O 2M_CODE suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/2M_CODE/themes/*.json` ou `$XDG_CONFIG_HOME/2M_CODE/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.2M_CODE/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.2M_CODE/themes/*.json`
