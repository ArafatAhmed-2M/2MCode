...
O 2M_CODE Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

:::note
O 2M_CODE Go está atualmente em beta.
:::

O Go funciona como qualquer outro provedor no 2M_CODE. Você assina o 2M_CODE Go e
...
O 2M_CODE Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O 2M_CODE Go funciona como qualquer outro provedor no 2M_CODE.

1. Você entra no **<a href={console}>2M_CODE Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `2M_CODE Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o 2M_CODE Go.
:::
...
O 2M_CODE Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do 2M_CODE
usa o formato `2M_CODE-go/<model-id>`. Por exemplo, para o Kimi K2.6, você usaria
`2M_CODE-go/kimi-k2.6` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o 2M_CODE também.
