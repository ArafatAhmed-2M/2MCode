...
description: Crie ferramentas que o LLM pode chamar no 2M_CODE.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do 2M_CODE, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.2M_CODE/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/2M_CODE/tools/`.
