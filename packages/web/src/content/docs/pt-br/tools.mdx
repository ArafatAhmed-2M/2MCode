...
As ferramentas permitem que o LLM execute ações em sua base de código. O 2M_CODE vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no 2M_CODE.
...
:::note
Esta ferramenta está disponível apenas quando `2M_CODE_EXPERIMENTAL_LSP_TOOL=true` (ou `2M_CODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor 2M_CODE ou quando a variável de ambiente `2M_CODE_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o 2M_CODE:
