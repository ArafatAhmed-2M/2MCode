...
O 2M_CODE tem uma lista de atalhos de teclado que você pode personalizar através de `tui.json`.
...
O 2M_CODE usa uma tecla `leader` para a maioria dos atalhos de teclado. Isso evita conflitos no seu terminal.
...
O prompt de entrada do aplicativo desktop 2M_CODE suporta atalhos comuns de estilo Readline/Emacs para edição de texto. Estes são embutidos e atualmente não são configuráveis via `2M_CODE.json`.
