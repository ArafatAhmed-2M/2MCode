...
Para depurar problemas com o 2M_CODE, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\2M_CODE\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `2M_CODE --log-level DEBUG`.

---

## Armazenamento

O 2M_CODE armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/2M_CODE/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\2M_CODE`
...
O 2M_CODE Desktop executa um servidor 2M_CODE local (o sidecar `2M_CODE-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `2M_CODE` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/2M_CODE/2M_CODE.jsonc` (ou `~/.config/2M_CODE/2M_CODE.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/2M_CODE/2M_CODE.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\2M_CODE\2M_CODE.jsonc`
...
O 2M_CODE também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/2M_CODE/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\2M_CODE\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o 2M_CODE possa reconstruí-lo.

1. Saia completamente do 2M_CODE Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/2M_CODE`
- **Linux**: exclua `~/.cache/2M_CODE` (ou execute `rm -rf ~/.cache/2M_CODE`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\2M_CODE`

3. Reinicie o 2M_CODE Desktop.

---

### Corrigindo problemas de conexão com o servidor

O 2M_CODE Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `2M_CODE.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `2M_CODE_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `2M_CODE_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o 2M_CODE Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do 2M_CODE.

---

### Notificações não aparecendo

O 2M_CODE Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o 2M_CODE nas configurações do seu sistema operacional, e
...
1. Saia do 2M_CODE Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo 2M_CODE Desktop):

- `2M_CODE.settings.dat` (URL do servidor padrão do desktop)
- `2M_CODE.global.dat` e `2M_CODE.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o 2M_CODE:
...
Para descobrir quais modelos você tem acesso, execute `2M_CODE models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\2M_CODE`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O 2M_CODE instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\2M_CODE`

2. Reinicie o 2M_CODE para reinstalar os pacotes de provedores mais recentes

Isso forçará o 2M_CODE a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O 2M_CODE detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
