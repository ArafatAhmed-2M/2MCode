...
`.2M_CODE/commands/test.md` を作成します。
...
カスタムコマンドは、2M_CODE 設定を通じて、または `commands/` ディレクトリに Markdown ファイルを作成することによって追加できます。

---

### JSON

2M_CODE [設定](/docs/config)で `command` オプションを使用します:
...
- グローバル: `~/.config/2M_CODE/commands/`
- プロジェクトごと: `.2M_CODE/commands/`
...
2M_CODE には、`/init`、`/undo`、`/redo`、`/share`、`/help` などのいくつかの組み込みコマンドが含まれています。 [詳細はこちら](/docs/tui#commands)。
