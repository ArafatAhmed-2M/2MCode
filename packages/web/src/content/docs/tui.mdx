---
title: TUI
description: Using the 2M_CODE terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

2M_CODE provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running 2M_CODE starts the TUI for the current directory.
...
When using the 2M_CODE TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to 2M_CODE. Allows you to select from available providers and add their API keys.
...
This is separate from `2M_CODE.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long 2M_CODE waits after the leader key. Defaults to `2000`.
...
Use `2M_CODE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `2M_CODE.default`.
