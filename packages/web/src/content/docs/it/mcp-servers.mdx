...
Puoi aggiungere strumenti esterni a 2M_CODE usando il _Model Context Protocol_, o MCP. 2M_CODE supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [2M_CODE Config](https://2M_CODE.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/2M_CODE`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
2M_CODE gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, 2M_CODE:
...
Se il server richiede autenticazione, 2M_CODE ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `2M_CODE mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, 2M_CODE memorizzera' i token in modo sicuro in `~/.local/share/2M_CODE/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in 2M_CODE insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione 2M_CODE come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare 2M_CODE al tuo account Sentry.
