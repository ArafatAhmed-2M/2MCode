---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di 2M_CODE.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

2M_CODE fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire 2M_CODE avvia la TUI per la directory corrente.
...
Quando usi la TUI di 2M_CODE, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a 2M_CODE. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da 2M_CODE. _Alias_: `/quit`, `/q`
...
Questo è separato da `2M_CODE.json`, che configura il comportamento di server/runtime.
...
Usa `2M_CODE_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
