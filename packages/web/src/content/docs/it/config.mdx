...
description: Usa la configurazione JSON di 2M_CODE.
---

Puoi configurare 2M_CODE usando un file di configurazione JSON.

---

## Formato

2M_CODE supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/2M_CODE`) - default dell'organizzazione
2. **Config globale** (`~/.config/2M_CODE/2M_CODE.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `2M_CODE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`2M_CODE.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.2M_CODE`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `2M_CODE_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.2M_CODE` e `~/.config/2M_CODE` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/2M_CODE`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di 2M_CODE in `~/.config/2M_CODE/2M_CODE.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/2M_CODE/tui.json`.
...
Aggiungi `2M_CODE.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando 2M_CODE si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `2M_CODE_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `2M_CODE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.2M_CODE` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.2M_CODE`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`2M_CODE.ai/config.json`**](https://2M_CODE.ai/config.json).

La configurazione TUI usa [**`2M_CODE.ai/tui.json`**](https://2M_CODE.ai/tui.json).
...
Usa `2M_CODE_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `2M_CODE.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `2M_CODE serve` e `2M_CODE web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server 2M_CODE.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `2M_CODE.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in 2M_CODE tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, 2M_CODE prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/2M_CODE/agents/` o `.2M_CODE/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, 2M_CODE fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`2M_CODE run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/2M_CODE/commands/` o `.2M_CODE/commands/`. [Scopri di piu](/docs/commands).
...
2M_CODE scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, 2M_CODE **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono 2M_CODE con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.2M_CODE/plugins/` o `~/.config/2M_CODE/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare 2M_CODE a usare solo alcuni provider invece di disabilitarli uno per uno.
