---
title: Provider
description: Usa qualsiasi provider LLM in 2M_CODE.
---

import config from "../../../../config.mjs"
export const console = config.console

2M_CODE usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di 2M_CODE.
...
2M_CODE Zen e una lista di modelli forniti dal team 2M_CODE, testati e
verificati per funzionare bene con 2M_CODE. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da 2M_CODE Zen.
:::

1. Esegui `/connect` nella TUI, seleziona 2M_CODE e vai su [2M_CODE.ai/auth](https://2M_CODE.ai/auth).
...
Funziona come qualsiasi altro provider in 2M_CODE ed e totalmente opzionale.
...
Per usare Amazon Bedrock con 2M_CODE:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui 2M_CODE:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `2M_CODE.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in 2M_CODE non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare 2M_CODE per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché 2M_CODE funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di 2M_CODE.
...
salvare il token nello storage di auth di 2M_CODE.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
2M_CODE usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare 2M_CODE
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `2M_CODE.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Personalizza tramite `2M_CODE.json`:
...
Per usare il tuo abbonamento GitHub Copilot con 2M_CODE:
...
Per usare Google Vertex AI con 2M_CODE:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui 2M_CODE.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da 2M_CODE, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [2M_CODE-helicone-session](https://github.com/H2Shami/2M_CODE-helicone-session) per loggare automaticamente ogni conversazione di 2M_CODE come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di 2M_CODE elencata come sessione separata.
...
Puoi configurare 2M_CODE per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare 2M_CODE per usare modelli locali tramite LM Studio.
...
Puoi configurare 2M_CODE per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per 2M_CODE. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/2M_CODE) per dettagli.
:::
...
Per usare Ollama Cloud con 2M_CODE:
...
3. Copia la chiave API da usare in 2M_CODE.
...
6. **Importante**: Prima di usare modelli cloud in 2M_CODE, devi scaricare le informazioni del modello localmente:
...
2M_CODE Zen è una lista di modelli testati e verificati forniti dal team 2M_CODE. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>2M_CODE Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **2M_CODE Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di 2M_CODE.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di 2M_CODE.
...
5. Puoi anche personalizzarli tramite la configurazione di 2M_CODE. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con 2M_CODE:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di 2M_CODE. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con 2M_CODE. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `2M_CODE.json` nella directory del progetto:
...
I campi `limit` permettono a 2M_CODE di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `2M_CODE auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di 2M_CODE e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config 2M_CODE.
