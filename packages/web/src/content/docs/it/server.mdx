---
title: Server
description: Interagisci con il server 2M_CODE via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `2M_CODE serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client 2M_CODE.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `2M_CODE.local` |
...
Imposta `2M_CODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `2M_CODE`, oppure imposta `2M_CODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `2M_CODE serve` sia per `2M_CODE web`.
...
Quando esegui `2M_CODE` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server 2M_CODE per interagire con 2M_CODE in modo programmatico.
:::

Questa architettura permette a 2M_CODE di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `2M_CODE serve` per avviare un server standalone. Se la TUI di 2M_CODE e gia in esecuzione, `2M_CODE serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di 2M_CODE.
...
Il server 2M_CODE espone le seguenti API.
