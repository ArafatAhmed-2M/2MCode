...
Per diagnosticare problemi con 2M_CODE, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\2M_CODE\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `2M_CODE --log-level DEBUG`.

---

## Archiviazione

2M_CODE salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/2M_CODE/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\2M_CODE`
...
2M_CODE Desktop esegue in background un server locale di 2M_CODE (il sidecar `2M_CODE-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `2M_CODE` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/2M_CODE/2M_CODE.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\2M_CODE\2M_CODE.jsonc`
...
2M_CODE puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/2M_CODE/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\2M_CODE\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che 2M_CODE possa ricostruirla.

1. Chiudi completamente 2M_CODE Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/2M_CODE`
- **Linux**: elimina `~/.cache/2M_CODE` (oppure esegui `rm -rf ~/.cache/2M_CODE`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\2M_CODE`

3. Riavvia 2M_CODE Desktop.

---

### Risolvi problemi di connessione al server

2M_CODE Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `2M_CODE.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `2M_CODE_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `2M_CODE_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, 2M_CODE Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di 2M_CODE.

---

### Notifiche non visualizzate

2M_CODE Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per 2M_CODE nelle impostazioni del sistema operativo, e
...
1. Chiudi 2M_CODE Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app 2M_CODE Desktop):

- `2M_CODE.settings.dat` (desktop default server URL)
- `2M_CODE.global.dat` e `2M_CODE.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con 2M_CODE:
...
Per capire a quali modelli hai accesso, esegui `2M_CODE models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\2M_CODE`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. 2M_CODE installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\2M_CODE`

2. Riavvia 2M_CODE per reinstallare i pacchetti provider piu recenti

Questo forzera 2M_CODE a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
2M_CODE rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
