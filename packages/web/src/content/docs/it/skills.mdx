...
Le skill degli agenti permettono a 2M_CODE di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
2M_CODE cerca in queste posizioni:

- Config di progetto: `.2M_CODE/skills/<name>/SKILL.md`
- Config globale: `~/.config/2M_CODE/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, 2M_CODE risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.2M_CODE/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/2M_CODE/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.2M_CODE/skills/git-release/SKILL.md` cosi':
...
2M_CODE elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `2M_CODE.json`:
...
**Per agenti integrati** (in `2M_CODE.json`):
