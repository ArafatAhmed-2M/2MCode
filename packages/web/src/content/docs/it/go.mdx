...
2M_CODE Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

:::note
2M_CODE Go è attualmente in beta.
:::

Go funziona come qualsiasi altro provider in 2M_CODE. Ti abboni a 2M_CODE Go e
...
2M_CODE Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

2M_CODE Go funziona come qualsiasi altro provider in 2M_CODE.

1. Accedi a **<a href={console}>2M_CODE Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `2M_CODE Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a 2M_CODE Go.
:::
...
2M_CODE Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua 2M_CODE config
utilizza il formato `2M_CODE-go/<model-id>`. Ad esempio, per Kimi K2.6, useresti
`2M_CODE-go/kimi-k2.6` nella tua configurazione.
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con 2M_CODE.
