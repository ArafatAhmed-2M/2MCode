...
In 2M_CODE esistono due tipi di agenti: agenti primari e subagenti.
...
2M_CODE include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
2M_CODE include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

2M_CODE include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di 2M_CODE, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Configura gli agenti nel file `2M_CODE.json`:
...
- Globale: `~/.config/2M_CODE/agents/`
- Per progetto: `.2M_CODE/agents/`
...
Se non specifichi una temperature, 2M_CODE usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di 2M_CODE sia per la config specifica del progetto.
...
L'ID modello nella configurazione 2M_CODE usa il formato `provider/model-id`. Per esempio, se stai usando [2M_CODE Zen](/docs/zen), useresti `2M_CODE/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `2M_CODE models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/ArafatAhmed-2M/2mcode).
:::
