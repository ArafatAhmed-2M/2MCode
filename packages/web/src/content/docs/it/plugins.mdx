---
title: Plugin
description: Scrivi plugin per estendere 2M_CODE.
---

I plugin ti permettono di estendere 2M_CODE agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di 2M_CODE.
...
- `.2M_CODE/plugins/` - plugin a livello progetto
- `~/.config/2M_CODE/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/2M_CODE/node_modules/`.
...
1. Config globale (`~/.config/2M_CODE/2M_CODE.json`)
2. Config di progetto (`2M_CODE.json`)
3. Directory plugin globale (`~/.config/2M_CODE/plugins/`)
4. Directory plugin di progetto (`.2M_CODE/plugins/`)
...
2M_CODE esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di 2M_CODE per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere 2M_CODE.
...
:::note
Se usi l'app desktop di 2M_CODE, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a 2M_CODE di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a 2M_CODE:
...
L'helper `tool` crea uno strumento personalizzato che 2M_CODE può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in 2M_CODE insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://2M_CODE.ai/docs/sdk) per i dettagli.
