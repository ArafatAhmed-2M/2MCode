...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. 2M_CODE include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in 2M_CODE.
...
:::note
Questo strumento e' disponibile solo quando `2M_CODE_EXPERIMENTAL_LSP_TOOL=true` (oppure `2M_CODE_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider 2M_CODE o quando la variabile d'ambiente `2M_CODE_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di 2M_CODE:
