---
title: LSP Servers
description: 2M_CODE integrates with your LSP servers.
---

2M_CODE can integrate with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM.

---

## Built-in

2M_CODE comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `2M_CODE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and 2M_CODE opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your 2M_CODE config.
