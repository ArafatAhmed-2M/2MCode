---
title: Providers
description: Using any LLM provider in 2M_CODE.
---

import config from "../../../config.mjs"
export const console = config.console

2M_CODE uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your 2M_CODE config.
...
in `~/.local/share/2M_CODE/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your 2M_CODE
config.
...
2M_CODE Zen is a list of models provided by the 2M_CODE team that have been
tested and verified to work well with 2M_CODE. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with 2M_CODE Zen.
:::

1. Run the `/connect` command in the TUI, select `2M_CODE Zen`, and head to [2M_CODE.ai/auth](https://2M_CODE.ai/zen).
...
It works like any other provider in 2M_CODE and is completely optional to use.

---

## 2M_CODE Go

2M_CODE Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the 2M_CODE team that have been
tested and verified to work well with 2M_CODE.

1. Run the `/connect` command in the TUI, select `2M_CODE Go`, and head to [2M_CODE.ai/auth](https://2M_CODE.ai/zen).
...
To use Amazon Bedrock with 2M_CODE:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running 2M_CODE:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY 2M_CODE

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile 2M_CODE

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX 2M_CODE
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `2M_CODE.json`:
...
2M_CODE. Anthropic explicitly prohibits this.

Previous versions of 2M_CODE came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in 2M_CODE with zero setup:
...
You can configure 2M_CODE to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="2M_CODE.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://2M_CODE.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for 2M_CODE to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX 2M_CODE
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX 2M_CODE
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your 2M_CODE config.
...
2M_CODE supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; 2M_CODE auto-creates a Model Access Key and discovers your available Models & Inference Routers.
...
3. Your browser opens to authorize 2M_CODE. Sign in and approve.

   :::note
   2M_CODE creates a Model Access Key named `2M_CODE-oauth-<timestamp>` in your DigitalOcean account. You can rotate or revoke it from the **Model Access Keys** page in the "Manage" section of the DigitalOcean console under Inference.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, 2M_CODE surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — 2M_CODE forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in 2M_CODE is experimental. Features, configuration, and
behavior may change in future releases.
:::

2M_CODE integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in 2M_CODE auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
2M_CODE uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock 2M_CODE
to only use your own GitLab-hosted instance, add the following to your
`2M_CODE.json` file. It is also recommended to disable session sharing.
...
Customize through `2M_CODE.json`:
...
When a `duo-workflow-*` model is selected, 2M_CODE will:
...
4. Route tool execution requests through 2M_CODE's permission-gated tool system
...
To use your GitHub Copilot subscription with 2M_CODE:
...
To use Google Vertex AI with 2M_CODE:
...
2. Set the required environment variables:
   ...
   Set them while running 2M_CODE.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id 2M_CODE
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through 2M_CODE, you can always configure it yourself.
...
```jsonc title="~/.config/2M_CODE/2M_CODE.jsonc"
{
  "$schema": "https://2M_CODE.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [2M_CODE-helicone-session](https://github.com/H2Shami/2M_CODE-helicone-session) plugin to automatically log each 2M_CODE conversation as a session in Helicone.

```bash
npm install -g 2M_CODE-helicone-session
```
...
```json title="2M_CODE.json"
{
  "plugin": ["2M_CODE-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each 2M_CODE conversation listed as a separate session.
...
You can configure 2M_CODE to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="2M_CODE.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://2M_CODE.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure 2M_CODE to use local models through LM Studio.

```json title="2M_CODE.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://2M_CODE.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure 2M_CODE to use local models through Ollama.

:::tip
Ollama can automatically configure itself for 2M_CODE. See the [Ollama integration docs](https://docs.ollama.com/integrations/2M_CODE) for details.
:::

```json title="2M_CODE.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://2M_CODE.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with 2M_CODE:
...
3. Copy the API key for use in 2M_CODE.
...
6. **Important**: Before using cloud models in 2M_CODE, you must pull the model information locally:
...
2M_CODE Zen is a list of tested and verified models provided by the 2M_CODE team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>2M_CODE Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **2M_CODE Zen**.

   ```txt
   /connect
   ```

3. Enter your 2M_CODE API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your 2M_CODE config.
...
5. You can also customize them through your 2M_CODE config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' 2M_CODE
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group 2M_CODE
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with 2M_CODE:
...
You can also customize models through your 2M_CODE config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with 2M_CODE. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in 2M_CODE.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `2M_CODE.json` file in your project directory:

   ```json title="2M_CODE.json" ""myprovider"" {5-15}
   {
     "$schema": "https://2M_CODE.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="2M_CODE.json" {9,11,17-20}
{
  "$schema": "https://2M_CODE.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow 2M_CODE to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `2M_CODE auth list` to see if the credentials
...
2. For custom providers, check the 2M_CODE config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your 2M_CODE config.
