---
title: Custom Tools
description: Create tools the LLM can call in 2M_CODE.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside 2M_CODE's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.2M_CODE/tools/` directory of your project.
- Or globally, by placing them in `~/.config/2M_CODE/tools/`.
