---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i 2M_CODE.
---

import config from "../../../../config.mjs"
export const console = config.console

2M_CODE bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i 2M_CODE-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/2M_CODE/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i 2M_CODE-konfigurationen.
...
2M_CODE Zen er en liste over modeller leveret af 2M_CODE-teamet, som er blevet testet og verificeret til at fungere godt med 2M_CODE. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med 2M_CODE Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg 2M_CODE og gå til [2M_CODE.ai/auth](https://2M_CODE.ai/auth).
...
Det fungerer som alle andre udbydere i 2M_CODE og er helt valgfrit at bruge.

---

## 2M_CODE Go

2M_CODE Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af 2M_CODE-teamet, som er testet og verificeret til at fungere godt med 2M_CODE.

1. Kør kommandoen `/connect` i TUI, vælg `2M_CODE Go`, og gå til [2M_CODE.ai/auth](https://2M_CODE.ai/zen).
...
Sådan bruger du Amazon Bedrock med 2M_CODE:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører 2M_CODE:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `2M_CODE.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i 2M_CODE understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere 2M_CODE til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at 2M_CODE skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem 2M_CODE-konfigurationen.
...
gemme tokenet i 2M_CODE auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
2M_CODE bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse 2M_CODE
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`2M_CODE.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med 2M_CODE:
...
Sådan bruger du Google Vertex AI med 2M_CODE:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem 2M_CODE, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [2M_CODE-helicone-session](https://github.com/H2Shami/2M_CODE-helicone-session) plugin for automatisk at logge hver 2M_CODE-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver 2M_CODE-samtale opført som en separat session.
...
Du kan konfigurere 2M_CODE til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere 2M_CODE til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere 2M_CODE til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for 2M_CODE. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/2M_CODE) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med 2M_CODE:
...
3. Kopier API-nøglen til brug i 2M_CODE.
...
6. **Vigtigt**: Før du bruger skymodeller i 2M_CODE, skal du hente modelinformationen lokalt:
...
2M_CODE Zen er en liste over testede og verificerede modeller leveret af 2M_CODE-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>2M_CODE Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **2M_CODE Zen**.

   ```txt
   /connect
   ```

3. Indtast 2M_CODE API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem 2M_CODE-konfigurationen.
...
5. Du kan også tilpasse dem gennem 2M_CODE-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med 2M_CODE:
...
Du kan også tilpasse modeller gennem 2M_CODE-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med 2M_CODE. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `2M_CODE.json`-filen i projektkataloget:
...
`limit`-felterne lader 2M_CODE forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `2M_CODE auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek 2M_CODE-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i 2M_CODE-konfigurationen.
