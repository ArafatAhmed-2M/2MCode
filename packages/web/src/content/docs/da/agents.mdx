...
Der er to typer agenter i 2M_CODE; primære agenter og subagenter.
...
2M_CODE leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
2M_CODE leveres med tre indbyggede subagenter, **General**, **Explore** og **Scout**. Vi ser nærmere på dem nedenfor.

---

## Indbyggede

2M_CODE leveres med to indbyggede primære agenter og tre indbyggede subagenter.
...
En skrivebeskyttet agent til eksterne docs og research af dependencies. Brug denne, når du har brug for at klone et dependency-repository ind i 2M_CODE's administrerede cache, inspicere kildekoden i et bibliotek eller krydstjekke lokal kode mod upstream-implementeringer uden at ændre dit workspace.
...
Konfigurer agent i din `2M_CODE.json`-konfigurationsfil:
...
- Globalt: `~/.config/2M_CODE/agents/`
- Pr. projekt: `.2M_CODE/agents/`
...
Hvis ingen temperatur er angivet, bruger 2M_CODE modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale 2M_CODE-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din 2M_CODE-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [2M_CODE Zen](/docs/zen), vil du bruge `2M_CODE/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `2M_CODE models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/ArafatAhmed-2M/2mcode).
:::
