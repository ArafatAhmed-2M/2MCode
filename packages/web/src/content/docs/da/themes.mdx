...
Med 2M_CODE kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger 2M_CODE vårt eget `2M_CODE`-tema.
...
2M_CODE kommer med flere innebygde temaer.
...
- Vil at 2M_CODE skal matche terminalens utseende
...
2M_CODE støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/2M_CODE/themes/*.json` eller `$XDG_CONFIG_HOME/2M_CODE/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.2M_CODE/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.2M_CODE/themes/*.json`
