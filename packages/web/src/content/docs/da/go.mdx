...
2M_CODE Go er et lavprisabonnement — **$5 for din første måned**, derefter **$10/måned** — der giver dig pålidelig adgang til populære åbne kodningsmodeller.

:::note
2M_CODE Go er i øjeblikket i beta.
:::

Go fungerer som enhver anden udbyder i 2M_CODE. Du abonnerer på 2M_CODE Go og
...
:::tip
Vi testede en udvalgt gruppe af modeller og udbydere, der fungerer godt med 2M_CODE.
:::
...
2M_CODE Go giver dig adgang til disse modeller for **$5 for din første måned**, derefter **$10/måned**.

---

## Sådan fungerer det

2M_CODE Go fungerer som enhver anden udbyder i 2M_CODE.

1. Du logger ind på **<a href={console}>2M_CODE Zen</a>**, abonnerer på Go, og
   kopierer din API-nøgle.
2. Du kører kommandoen `/connect` i TUI'en, vælger `2M_CODE Go`, og indsætter
   din API-nøgle.
3. Kør `/models` i TUI'en for at se listen over tilgængelige modeller gennem Go.

:::note
Kun ét medlem per arbejdsområde kan abonnere på 2M_CODE Go.
:::
...
2M_CODE Go inkluderer følgende grænser:
...
Dit [model id](/docs/config/#models) i din 2M_CODE config
bruger formatet `2M_CODE-go/<model-id>`. For eksempel for Kimi K2.6, vil du
bruge `2M_CODE-go/kimi-k2.6` i din config.
...
4. Undgå **lock-in** ved også at lade dig bruge enhver anden udbyder med 2M_CODE.
