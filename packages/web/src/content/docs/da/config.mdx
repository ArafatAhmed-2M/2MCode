---
title: Konfiguration
description: Ved at bruge 2M_CODE JSON konfig.
---

Du kan konfigurere 2M_CODE ved hjælp af en JSON-konfigurationsfil.

---

## Format

2M_CODE understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/2M_CODE`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/2M_CODE/2M_CODE.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`2M_CODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`2M_CODE.json` i projekt) - projektspecifikke indstillinger
5. **`.2M_CODE` mapper** - agent, kommandoer, plugins
6. **Inline config** (`2M_CODE_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.2M_CODE` og `~/.config/2M_CODE` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/2M_CODE`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale 2M_CODE-konfiguration i `~/.config/2M_CODE/2M_CODE.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `2M_CODE.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når 2M_CODE starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `2M_CODE_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `2M_CODE_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.2M_CODE` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.2M_CODE` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`2M_CODE.ai/config.json`**](https://2M_CODE.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `2M_CODE serve` og `2M_CODE web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din 2M_CODE-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `2M_CODE.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din 2M_CODE-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger 2M_CODE at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din 2M_CODE-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/2M_CODE/agents/` eller `.2M_CODE/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil 2M_CODE falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`2M_CODE run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/2M_CODE/commands/` eller `.2M_CODE/commands/`. [Learn more here](/docs/commands).
...
2M_CODE vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard 2M_CODE **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide 2M_CODE med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.2M_CODE/plugins/` eller `~/.config/2M_CODE/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse 2M_CODE til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
