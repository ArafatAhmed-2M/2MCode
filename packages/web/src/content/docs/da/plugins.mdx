---
title: Plugins
description: Skriv dine egne plugins for at udvide 2M_CODE.
---

Plugins giver dig mulighed for at udvide 2M_CODE ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre 2M_CODEs standardadfærd.
...
- `.2M_CODE/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/2M_CODE/node_modules/`.
...
1. Global konfiguration (`~/.config/2M_CODE/2M_CODE.json`)
2. Projektkonfiguration (`2M_CODE.json`)
3. Global plugin-mappe (`~/.config/2M_CODE/plugins/`)
4. Projekt plugin bibliotek (`.2M_CODE/plugins/`)
...
2M_CODE kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En 2M_CODE SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide 2M_CODE.
...
:::note
Hvis du bruger 2M_CODE desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå 2M_CODE i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til 2M_CODE:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som 2M_CODE kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for 2M_CODE sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://2M_CODE.ai/docs/sdk) for detaljer.
