...
Du kan tilføje eksterne værktøjer til 2M_CODE ved hjælp af _Model Context Protocol_ eller MCP. 2M_CODE understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [2M_CODE Config](https://2M_CODE.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/2M_CODE`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
2M_CODE håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil 2M_CODE:
...
Hvis serveren kræver godkendelse, vil 2M_CODE bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `2M_CODE mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil 2M_CODE opbevare tokens sikkert i `~/.local/share/2M_CODE/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem 2M_CODE-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde 2M_CODE til din Sentry-konto.
