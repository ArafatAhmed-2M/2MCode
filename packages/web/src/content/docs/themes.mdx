...
With 2M_CODE you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, 2M_CODE uses our own `2M_CODE` theme.
...
2M_CODE comes with several built-in themes.
...
- Want 2M_CODE to match their terminal's appearance
...
2M_CODE supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/2M_CODE/themes/*.json` or `$XDG_CONFIG_HOME/2M_CODE/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.2M_CODE/themes/*.json`
4. **Current working directory** - Defined in `./.2M_CODE/themes/*.json`
