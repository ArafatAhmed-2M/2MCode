...
description: Cree herramientas que LLM pueda llamar en 2M_CODE.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de 2M_CODE como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.2M_CODE/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/2M_CODE/tools/`.
...
```ts title=".2M_CODE/tools/database.ts" {1}
...
```ts title=".2M_CODE/tools/math.ts"
import { tool } from "@2mcode-ai/plugin"
...
```ts title=".2M_CODE/tools/bash.ts"
import { tool } from "@2mcode-ai/plugin"
...
```ts title=".2M_CODE/tools/project.ts" {8}
import { tool } from "@2mcode-ai/plugin"
...
```python title=".2M_CODE/tools/add.py"
import sys
...
```ts title=".2M_CODE/tools/python-add.ts" {10}
import { tool } from "@2mcode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".2M_CODE/tools/add.py")
