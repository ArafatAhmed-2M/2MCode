...
Hay dos tipos de agentes en 2M_CODE; agentes primarios y subagentes.
...
2M_CODE viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
2M_CODE viene con tres subagentes integrados, **General**, **Explore** y **Scout**. Veremos esto a continuación.

---

## Integrados

2M_CODE viene con dos agentes primarios integrados y tres subagentes integrados.
...
Configure agentes en su archivo de configuración `2M_CODE.json`:
...
Si no se especifica ninguna temperatura, 2M_CODE utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global 2M_CODE como para la configuración específica del proyecto.
...
El ID del modelo en su configuración 2M_CODE usa el formato `provider/model-id`. Por ejemplo, si usa [2M_CODE Zen](/docs/zen), usaría `2M_CODE/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Ejecute `2M_CODE models` para ver una lista de los modelos disponibles.
:::
...
:::tip
¿Tiene un agente que le gustaría compartir? [Enviar un PR](https://github.com/ArafatAhmed-2M/2mcode).
:::
