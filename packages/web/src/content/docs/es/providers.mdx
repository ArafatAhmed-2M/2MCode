---
title: Proveedores
description: Usando cualquier proveedor LLM en 2M_CODE.
---

import config from "../../../../config.mjs"
export const console = config.console

2M_CODE usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración 2M_CODE.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu 2M_CODE
configuración.
...
2M_CODE Zen es una lista de modelos proporcionados por el equipo 2M_CODE que han sido
probado y verificado para funcionar bien con 2M_CODE. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con 2M_CODE Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione 2M_CODE y diríjase a [2M_CODE.ai/auth](https://2M_CODE.ai/auth).
...
Funciona como cualquier otro proveedor en 2M_CODE y su uso es completamente opcional.

---

## 2M_CODE Go

2M_CODE Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de 2M_CODE que han sido
probado y verificado para funcionar bien con 2M_CODE.

1. Ejecute el comando `/connect` en TUI, seleccione `2M_CODE Go` y diríjase a [2M_CODE.ai/auth](https://2M_CODE.ai/zen).
...
Para usar Amazon Bedrock con 2M_CODE:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta 2M_CODE:
   ...
   Para una configuración persistente o específica del proyecto, utilice `2M_CODE.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en 2M_CODE no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar 2M_CODE para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que 2M_CODE funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración 2M_CODE.
...
para almacenar el token en el almacenamiento de autenticación 2M_CODE.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
2M_CODE utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear 2M_CODE
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `2M_CODE.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `2M_CODE.json`:
...
Para utilizar su suscripción GitHub Copilot con 2M_CODE:
...
Para utilizar Google Vertex AI con 2M_CODE:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta 2M_CODE.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de 2M_CODE, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [2M_CODE-helicone-session](https://github.com/H2Shami/2M_CODE-helicone-session) para registrar automáticamente cada conversación 2M_CODE como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación 2M_CODE enumerada como una sesión separada.
...
Puede configurar 2M_CODE para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar 2M_CODE para usar modelos locales a través de LM Studio.
...
Puedes configurar 2M_CODE para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para 2M_CODE. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/2M_CODE) para obtener más detalles.
:::
...
Para usar Ollama Cloud con 2M_CODE:
...
3. Copie la clave API para usarla en 2M_CODE.
...
6. **Importante**: Antes de usar modelos en la nube en 2M_CODE, debe obtener la información del modelo localmente:
...
2M_CODE Zen es una lista de modelos probados y verificados proporcionada por el equipo 2M_CODE. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>2M_CODE Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **2M_CODE Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave 2M_CODE API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración 2M_CODE.
...
5. También puedes personalizarlos a través de tu configuración 2M_CODE. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con 2M_CODE:
...
También puede personalizar modelos a través de su configuración 2M_CODE. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con 2M_CODE. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `2M_CODE.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a 2M_CODE comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `2M_CODE auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración 2M_CODE y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración 2M_CODE.
