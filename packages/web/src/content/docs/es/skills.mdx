...
Las habilidades del agente permiten a 2M_CODE descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
2M_CODE busca estas ubicaciones:

- Configuración del proyecto: `.2M_CODE/skills/<name>/SKILL.md`
- Configuración global: `~/.config/2M_CODE/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, 2M_CODE sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.2M_CODE/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/2M_CODE/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.2M_CODE/skills/git-release/SKILL.md` así:
...
2M_CODE enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `2M_CODE.json`:
...
**Para agentes integrados** (en `2M_CODE.json`):
