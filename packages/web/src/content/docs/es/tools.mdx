...
Las herramientas permiten que LLM realice acciones en su código base. 2M_CODE viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en 2M_CODE.
...
:::note
Esta herramienta solo está disponible cuando `2M_CODE_EXPERIMENTAL_LSP_TOOL=true` (o `2M_CODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor 2M_CODE o cuando la variable de entorno `2M_CODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar 2M_CODE:

```bash
2M_CODE_ENABLE_EXA=1 2M_CODE
```
