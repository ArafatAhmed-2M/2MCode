...
Puede agregar herramientas externas a 2M_CODE utilizando el _Model Context Protocol_ o MCP. 2M_CODE admite servidores locales y remotos.
...
Puede definir servidores MCP en su [2M_CODE Config](https://2M_CODE.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/2M_CODE`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
2M_CODE maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, 2M_CODE:
...
Si el servidor requiere autenticación, 2M_CODE le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `2M_CODE mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, 2M_CODE almacenará los tokens de forma segura en `~/.local/share/2M_CODE/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en 2M_CODE, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración 2M_CODE como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar 2M_CODE a su cuenta Sentry.
