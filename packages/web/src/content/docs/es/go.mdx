...
2M_CODE Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

:::note
2M_CODE Go está actualmente en beta.
:::

Go funciona como cualquier otro proveedor en 2M_CODE. Te suscribes a 2M_CODE Go y
...
2M_CODE Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

2M_CODE Go funciona como cualquier otro proveedor en 2M_CODE.

1. Inicias sesión en **<a href={console}>2M_CODE Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `2M_CODE Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a 2M_CODE Go.
:::
...
2M_CODE Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de 2M_CODE
usa el formato `2M_CODE-go/<model-id>`. Por ejemplo, para Kimi K2.6, usarías
`2M_CODE-go/kimi-k2.6` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con 2M_CODE.
