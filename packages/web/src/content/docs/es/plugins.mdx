---
title: Complementos
description: Escriba sus propios complementos para extender 2M_CODE.
---

Los complementos le permiten extender 2M_CODE al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de 2M_CODE.
...
- `.2M_CODE/plugins/` - Complementos a nivel de proyecto
- `~/.config/2M_CODE/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/2M_CODE/node_modules/`.
...
1. Configuración global (`~/.config/2M_CODE/2M_CODE.json`)
2. Configuración del proyecto (`2M_CODE.json`)
3. Directorio global de complementos (`~/.config/2M_CODE/plugins/`)
4. Directorio de complementos del proyecto (`.2M_CODE/plugins/`)
...
2M_CODE ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK 2M_CODE para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar 2M_CODE.
...
:::note
Si está utilizando la aplicación de escritorio 2M_CODE, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que 2M_CODE lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a 2M_CODE:

```ts title=".2M_CODE/plugins/custom-tools.ts"
import { type Plugin, tool } from "@2M_CODE-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que 2M_CODE puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para 2M_CODE junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://2M_CODE.ai/docs/sdk) para obtener más detalles.
