...
Con 2M_CODE puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, 2M_CODE usa nuestro propio tema `2M_CODE`.
...
2M_CODE viene con varios temas integrados.
...
- Quiere que 2M_CODE coincida con la apariencia de su terminal
...
2M_CODE admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/2M_CODE/themes/*.json` o `$XDG_CONFIG_HOME/2M_CODE/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.2M_CODE/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.2M_CODE/themes/*.json`
