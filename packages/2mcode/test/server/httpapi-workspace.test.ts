...
function request(path: string, directory: string, init: RequestInit = {}) {
  return Effect.promise(() => {
    ...
    headers.set("x-2M_CODE-directory", directory)
      ...
      try {
        const response = yield* request(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-2M_CODE-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://2M_CODE.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-2M_CODE-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-2M_CODE-workspace")
      } finally {
