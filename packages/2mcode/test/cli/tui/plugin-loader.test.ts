...
import type { TuiAttentionSoundPack } from "@2mcode-ai/plugin/tui"
...
`,
  ...
  process.env._2MCODE_PLUGIN_META_FILE = path.join(tmp.path, "plugin-meta.json")
  const config = createTuiResolvedConfig({
    plugin: [
...
`,
    ...
    const list = await Filesystem.readJson<Record<string, { themes?: Record<string, { dest: string }> }>>(
      process.env._2MCODE_PLUGIN_META_FILE!,
    )
