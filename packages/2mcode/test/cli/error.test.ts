...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/2M_CODE.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/2M_CODE.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/2M_CODE.jsonc", dir: ".2M_CODE", suggestion: "2M_CODE" },
        expected:
          'Directory ".2M_CODE" in /tmp/2M_CODE.jsonc is not valid. Rename the directory to "2M_CODE" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/2M_CODE.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/2M_CODE.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.2M_CODE.ai/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.2M_CODE.ai/auth/device/code.")
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `2M_CODE models` to list available models",
      "Or check your config (2M_CODE.json) provider/model names",
    ].join("\n")
