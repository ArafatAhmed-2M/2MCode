...
const globalConfigFiles = ["2M_CODE.json", "2M_CODE.jsonc", "tui.json", "tui.jsonc"].map((file) =>
...
it.instance("keeps server and tui plugin merge semantics aligned", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      const local = path.join(test.directory, ".2M_CODE")
      yield* fs.makeDirectory(local, { recursive: true })

      yield* fs.writeJson(path.join(Global.Path.config, "2M_CODE.json"), {
        plugin: [["shared-plugin@1.0.0", { source: "global" }], "global-only@1.0.0"],
      })
      yield* fs.writeJson(path.join(Global.Path.config, "tui.json"), {
        plugin: [["shared-plugin@1.0.0", { source: "global" }], "global-only@1.0.0"],
      })
      yield* fs.writeJson(path.join(local, "2M_CODE.json"), {
  ...
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeWithDirs(
        path.join(test.directory, ".2M_CODE", "tui.json"),
...
it.instance("migrates tui-specific keys from 2M_CODE.json when tui.json does not exist", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
      const source = path.join(test.directory, "2M_CODE.json")
      ...
      expect(yield* fs.existsSafe(path.join(test.directory, "2M_CODE.json.tui-migration.bak"))).toBe(true)
...
it.instance("migrates project legacy tui keys even when global tui.json already exists", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      const test = yield* TestInstance
      yield* fs.writeJson(path.join(Global.Path.config, "tui.json"), { theme: "global" })
      yield* fs.writeJson(path.join(test.directory, "2M_CODE.json"), {
      ...
      const server = JSON.parse(yield* fs.readFileString(path.join(test.directory, "2M_CODE.json")))
      expect(server.theme).toBeUndefined()
      expect(server.tui).toBeUndefined()
    }),
  ),
...
it.instance("skips migration when 2M_CODE.jsonc is syntactically invalid", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
      yield* fs.writeFileString(
        path.join(test.directory, "2M_CODE.jsonc"),
        `{
  "theme": "broken-theme",
  "tui": { "scroll_speed": 2 }
  "username": "still-broken"
      ...
      expect(yield* fs.existsSafe(path.join(test.directory, "2M_CODE.jsonc.tui-migration.bak"))).toBe(false)
      const source = yield* fs.readFileString(path.join(test.directory, "2M_CODE.jsonc"))
...
it.instance("skips migration when tui.json already exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
      yield* fs.writeJson(path.join(test.directory, "2M_CODE.json"), { theme: "legacy" })
      ...
      expect(yield* fs.existsSafe(path.join(test.directory, "2M_CODE.json.tui-migration.bak"))).toBe(false)
    }),
  ),
)
...
it.instance("migration backup preserves JSONC comments", () =>
  ...
  "tui": {
    ...
    "scroll_speed": 1.5
      ...
      const backup = yield* fs.readFileString(path.join(test.directory, "2M_CODE.jsonc.tui-migration.bak"))
...
it.instance("migrates legacy tui keys across multiple 2M_CODE.json levels", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
      ...
      yield* fs.writeJson(path.join(test.directory, "2M_CODE.json"), { theme: "root-theme" })
      yield* fs.writeJson(path.join(nested, "2M_CODE.json"), { theme: "nested-theme" })
...
it.instance("does not derive tui path from 2M_CODE_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeJson(path.join(customDir, "2M_CODE.json"), { model: "test/model" })
...
it.instance("loads .2M_CODE/tui.json", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
...
it.instance("silently skips malformed tui.json - load failures degrade to {}", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeWithDirs(path.join(test.directory, ".2M_CODE", "tui.json"), JSON.stringify({ theme: "fallback" }))
