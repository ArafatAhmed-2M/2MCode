...
async function writeManagedSettings(settings: object, filename = "2M_CODE.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "2M_CODE.json") {
...
test("creates global jsonc config with schema when no global configs exist", async () => {
  ...
  try {
    ...
    const content = await Filesystem.readText(path.join(tmp.path, "2M_CODE.jsonc"))
    expect(content).toContain('"$schema": "https://2M_CODE.ai/config.json"')
  } finally {
    ;(Global.Path as { config: string }).config = prev
    await clear(true)
  }
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "2M_CODE.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "2M_CODE.jsonc")
...
test("ignores legacy tui keys in 2M_CODE config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://2M_CODE.ai/config.json",
  ...
  try {
    ...
    await withTestInstance({
      ...
      fn: async (ctx) => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "2M_CODE.json"))
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["2M_CODE"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "2M_CODE.json"), "{ invalid json }")
    },
  })
  await provideTestInstance({
    directory: tmp.path,
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".2M_CODE", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "2M_CODE.json"),
        JSON.stringify({ $schema: "https://2M_CODE.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".2M_CODE")
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/2M_CODE")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/2M_CODE")
    ...
    if (urlStr.includes(".well-known/2M_CODE")) {
      ...
      return Promise.resolve(
        new Response(
          JSON.stringify({
            remote_config: {
              url: "https://config.example.com/2M_CODE.json",
              headers: {
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(wellknownFetchedUrl).toBe("https://example.com/.well-known/2M_CODE")
            expect(remoteFetchedUrl).toBe("https://config.example.com/2M_CODE.json")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "2M_CODE.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-2M_CODE@2.4.3", file)).toBe("oh-my-2M_CODE@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-2M_CODE@2.4.3", "file:///project/.2M_CODE/plugin/oh-my-2M_CODE.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.2M_CODE/plugin/demo.ts", "file:///project/.2M_CODE/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.2M_CODE/plugin/demo.ts"])
  })
  ...
  const config = ConfigParse.schema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "2M_CODE Managed",
          PayloadIdentifier: "ai.2M_CODE.managed.test",
          PayloadType: "ai.2M_CODE.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://2M_CODE.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://2M_CODE.ai/config.json")
})
