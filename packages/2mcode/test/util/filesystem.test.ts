...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "2M_CODE.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "2M_CODE.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "2M_CODE.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "2M_CODE.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["2M_CODE.json", "2M_CODE.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "2M_CODE.json"),
        path.join(tmp.path, "2M_CODE.jsonc"),
        path.join(project, "2M_CODE.json"),
        path.join(project, "2M_CODE.jsonc"),
      ])
    })
  })
