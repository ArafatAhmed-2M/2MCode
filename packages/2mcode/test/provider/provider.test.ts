...
function paid(providers: Awaited<ReturnType<typeof list>>) {
  const item = providers[ProviderID.make("2M_CODE")]
...
test("ModelNotFoundError suggests catalog models for unloaded providers", async () => {
  ...
  await withTestInstance({
    directory: tmp.path,
    fn: async (ctx) => {
      remove(ctx, "_2MCODE_API_KEY")
      try {
        await getModel(ProviderID["2M_CODE"], ModelID.make("claude-haiku-fake-model"), ctx)
        ...
        JSON.stringify({
          ...
          provider: {
            "cloudflare-ai-gateway": {
              options: {
                metadata: { invoked_by: "test", project: "2M_CODE" },
              },
            },
          },
        }),
...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const configDir = path.join(dir, ".2M_CODE")
...
test("plugin config enabled and disabled providers are honored", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const root = path.join(dir, ".2M_CODE", "plugin")
      await mkdir(root, { recursive: true })
      await Bun.write(
...
test("2M_CODE loader keeps paid models when config apiKey is present", async () => {
...
test("2M_CODE loader keeps paid models when auth exists", async () => {
  await using base = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "2M_CODE.json"),
