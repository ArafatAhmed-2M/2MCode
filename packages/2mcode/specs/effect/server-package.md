...
- The server still lives in `packages/2M_CODE`.
...
Until enough shared service code lives outside `packages/2M_CODE`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/2M_CODE`

It should not import `packages/2M_CODE` services while `packages/2M_CODE`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on 2M_CODE-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/2M_CODE` implementation details.
