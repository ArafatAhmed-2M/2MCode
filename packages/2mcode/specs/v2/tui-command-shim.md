...
Delete `packages/2M_CODE/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/2M_CODE/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/2M_CODE/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/2M_CODE`
- TUI plugin loader tests in `packages/2M_CODE` if runtime plugin API wiring changed
