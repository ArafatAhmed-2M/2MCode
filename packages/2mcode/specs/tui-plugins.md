...
- Author package entrypoint is `@2mcode-ai/plugin/tui`.
...
- Import types from `@2mcode-ai/plugin/tui`.
- `@2mcode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@2mcode-ai/plugin/tui"
...
That is what makes local config-scoped plugins able to import `@2mcode-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@2mcode-ai/plugin/tui`.
