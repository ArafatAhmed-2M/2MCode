...
- Author package entrypoint is `@2M_CODE-ai/plugin/tui`.
...
```json
{
  "$schema": "https://2M_CODE.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/2M_CODE-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@2M_CODE-ai/plugin/tui`.
- `@2M_CODE-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@2M_CODE-ai/plugin/tui"
...
- The value is a semver range checked against the running 2M_CODE version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.2M_CODE` is absent, no check is performed (backward compatible).
...
- `2M_CODE plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `2M_CODE plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.2M_CODE` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.2M_CODE`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.2M_CODE`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.2M_CODE` dir or `2M_CODE_CONFIG_DIR`, 2M_CODE installs `@2M_CODE-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@2M_CODE-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus 2M_CODE's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@2M_CODE-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"2M_CODE"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `2M_CODE.default`.
...
- Local plugins persist installed themes under the local `.2M_CODE/themes` area near the plugin config source.
...
- Local smoke plugin: `.2M_CODE/plugins/tui-smoke.tsx`
- Local vim plugin: `.2M_CODE/plugins/tui-vim.tsx`
- Local smoke config: `.2M_CODE/tui.json`
- Local smoke theme: `.2M_CODE/plugins/smoke-theme.json`
