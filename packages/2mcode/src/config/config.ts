...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in 2M_CODE config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for 2M_CODE serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://2M_CODE.ai/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://2M_CODE.ai/docs/agents" }),
...
export class Service extends Context.Service<Service, Interface>()("@2M_CODE/Config") {}

function globalConfigFile() {
  const candidates = ["2M_CODE.jsonc", "2M_CODE.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://2M_CODE.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      if (!Flag._2MCODE_CONFIG && !Flag._2MCODE_CONFIG_DIR && !Flag._2MCODE_CONFIG_CONTENT) {
        ...
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://2M_CODE.ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "2M_CODE.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "2M_CODE.jsonc")))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://2M_CODE.ai/config.json"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/2M_CODE` })
            const response = yield* Effect.promise(() => fetch(`${url}/.well-known/2M_CODE`))
            if (!response.ok) {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://2M_CODE.ai/config.json"
            const source = `${url}/.well-known/2M_CODE`
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under `.2M_CODE/plugin(s)` are already local files, so ConfigPlugin.load
