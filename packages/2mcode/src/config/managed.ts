...
const MANAGED_PLIST_DOMAIN = "ai.2M_CODE.managed"

// Keys injected by macOS/MDM into the managed plist that are not 2M_CODE config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/2M_CODE"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "2M_CODE")
    default:
      return "/etc/2M_CODE"
  }
}
