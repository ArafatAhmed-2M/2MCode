...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for 2M_CODE.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal 2M_CODE sessions
...
2M_CODE acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via 2M_CODE's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | 2M_CODE acp
```
...
ACP sessions map cleanly to 2M_CODE's internal session model:
