...
//      2M_CODE server and runs interactive mode against it.
//
...
import { create2M_CODEClient, type 2M_CODEClient, type ToolPart } from "@2M_CODE-ai/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run 2M_CODE with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running 2M_CODE server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
