...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall 2M_CODE and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall 2M_CODE")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g 2M_CODE-ai",
      pnpm: "pnpm uninstall -g 2M_CODE-ai",
      bun: "bun remove -g 2M_CODE-ai",
      yarn: "yarn global remove 2M_CODE-ai",
      brew: "brew uninstall 2M_CODE",
      choco: "choco uninstall 2M_CODE",
      scoop: "scoop uninstall 2M_CODE",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "2M_CODE", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using 2M_CODE!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# 2M_CODE") || content.includes(".2M_CODE/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".2M_CODE/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".2M_CODE/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".2M_CODE"))
    ) {
      continue
    }
