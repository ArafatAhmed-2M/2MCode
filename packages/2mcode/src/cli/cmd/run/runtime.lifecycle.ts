...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "2M_CODE.agent.name": input.agent,
      "2M_CODE.directory": input.directory,
      "2M_CODE.first": input.first,
      "2M_CODE.model.provider": input.model?.providerID,
      "2M_CODE.model.id": input.model?.modelID,
      "2M_CODE.model.variant": input.variant,
      ...
      try {
        ...
        const close = async (next: {
          ...
          return withRunSpan(
            "RunLifecycle.close",
            {
              "2M_CODE.show_exit": next.showExit,
