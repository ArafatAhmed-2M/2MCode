...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "2M_CODE.mode": input.resolveSession ? "local" : "attach",
      "2M_CODE.initial_input": !!input.initialInput,
      "2M_CODE.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "2M_CODE.directory": ctx.directory,
        "2M_CODE.resume": ctx.resume === true,
        "2M_CODE.agent.name": state.agent,
        "2M_CODE.model.provider": state.model?.providerID,
        "2M_CODE.model.id": state.model?.modelID,
        "2M_CODE.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "2M_CODE.model.provider": model.providerID,
            "2M_CODE.model.id": model.modelID,
            "2M_CODE.model.variant": state.activeVariant,
          })
          return {
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "2M_CODE.model.id": state.model?.modelID,
                "2M_CODE.model.variant": state.activeVariant,
                "2M_CODE.prompt.chars": prompt.text.length,
                "2M_CODE.prompt.parts": prompt.parts.length,
                "2M_CODE.prompt.include_files": includeFiles,
                "2M_CODE.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "2M_CODE.directory": input.directory,
      "2M_CODE.initial_input": !!input.initialInput,
      "2M_CODE.demo": input.demo,
    },
    async () => {
