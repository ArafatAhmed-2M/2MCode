...
const AGENT_USERNAME = "2M_CODE-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/2M_CODE.yml"
    ...
    yield* Effect.promise(async () => {
      {
        ...
        function printNextSteps() {
          ...
          prompts.outro(
            [
              ...
              "",
              "   Learn more about the GitHub agent - https://2M_CODE.ai/docs/github/#usage-examples",
            ].join("\n"),
          )
        }
        ...
        async function installGitHubApp() {
          ...
          // Open browser
          const url = "https://github.com/apps/2M_CODE-agent"
          const command =
          ...
          async function getInstallation() {
            return await fetch(
              `https://api.2M_CODE.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
            )
              .then((res) => res.json())
...
jobs:
  "2M_CODE":
    if: |
      ...
      startsWith(github.event.comment.body, '/2M_CODE')
    ...
    steps:
      ...
      - name: Run 2M_CODE
        uses: anomalyco/2M_CODE/github@latest${envStr}
        with:
...
export const GithubRunCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.github.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.2M_CODE.ai" : "https://2M_CODE.ai"
      ...
      try {
        ...
        console.log("2M_CODE session", session.id)
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.2M_CODE.ai"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/2M_CODE,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to 2M_CODE...")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("2M_CODE-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `2M_CODE/${type}-${hex}-${timestamp}`
        }
        return `2M_CODE/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/2M_CODE-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[2M_CODE session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the 2M_CODE infrastructure after your response",
