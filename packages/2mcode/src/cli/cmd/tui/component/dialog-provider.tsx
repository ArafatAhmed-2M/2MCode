...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@2M_CODE-ai/sdk/v2"
...
const CUSTOM_PROVIDER_OPTION_VALUE = "__2M_CODE_custom_provider__"
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in 2M_CODE.json to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
      ...
      description={
        {
          2M_CODE: (
            <box gap={1}>
              ...
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://2M_CODE.ai/zen</span> to get a key
              </text>
            </box>
          ),
          "2M_CODE-go": (
            <box gap={1}>
              <text fg={theme.textMuted}>
                2M_CODE Go is a $10 per month subscription that provides reliable access to popular open coding models
                with generous usage limits.
              </text>
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://2M_CODE.ai/zen</span> and enable 2M_CODE Go
              </text>
            </box>
          ),
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure it in 2M_CODE.json to use it.`,
          })
          dialog.clear()
          return
        }
