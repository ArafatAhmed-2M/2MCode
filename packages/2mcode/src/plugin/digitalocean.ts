...
const MAK_NAME_PREFIX = "2M_CODE-oauth"
...
<html>
  <head>
    <meta charset="utf-8" />
    <title>2M_CODE - DigitalOcean Authorization</title>
    <style>
    ...
    <script>
      (async function() {
        ...
        try {
          ...
          msgEl.textContent = "You can close this window and return to 2M_CODE."
...
async function createModelAccessKey(bearer: string): Promise<ApiKeyInfo> {
  ...
  const res = await fetch(`${DO_API_BASE}/v2/gen-ai/models/api_keys`, {
    ...
    headers: {
      ...
      "User-Agent": `2M_CODE/${InstallationVersion}`,
    },
      ...
      methods: [
        {
          ...
          async authorize() {
            ...
            return {
              ...
              instructions:
                "Sign in to DigitalOcean in your browser. 2M_CODE will create a Model Access Key named 2M_CODE-oauth-* and load your Inference Routers. Re-run /connect to refresh routers later.",
