...
Replace runtime Bun shell template-tag usage in `packages/2M_CODE/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/2M_CODE/src` is removed except:
