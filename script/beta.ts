...
const model = "2M_CODE/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/2M_CODE")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with 2M_CODE...`)
  ...
  try {
    await $`2M_CODE run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  2M_CODE failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with 2M_CODE")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with 2M_CODE...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/2M_CODE`.",
