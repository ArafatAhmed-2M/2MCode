...
const repo = process.env.GH_REPO ?? "anomalyco/2M_CODE"
const bot = ["actions-user", "github-actions[bot]", "2M_CODE", "2M_CODE-agent[bot]"]
const team = [
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/2M_CODE packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/2M_CODE/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/2M_CODE/")) areas.add("core")
