...
async function main() {
  ...
  // Delete any existing branches for 2M_CODE updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating 2M_CODE extension to v${cleanVersion}"`
