...
`)
  ...
  const 2M_CODE = await create2M_CODE({ port: 0 })
  ...
  try {
    ...
    const session = await 2M_CODE.client.session.create()
    const result = await 2M_CODE.client.session
      .prompt({
